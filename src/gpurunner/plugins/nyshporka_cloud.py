...
class VastRent:
    ...
    def estimate(self, need: Any, *, target: str = "") -> dict[str, Any]:
        ...
        out.update({
            ...
            "pph_sure": float(round(best.pph_sure)),
        })
        ...
        if not score_need.lines_assumed:
    ...
    def _score_need(need: Any) -> tuple[Any, float]:
        ...
        """
        from gpurunner.core.htr_sizing import GB_PER_SHARD, LINES_PER_PAGE_ASSUMED
        ...
        # прогону). Нуль = не міряли: тоді вибір бере припущену густину
        # (`LINES_PER_PAGE_ASSUMED`), а не якір кривої, на якому ціль
        # недосяжна для будь-якої машини.
        lines = max(0.0, float(getattr(need, "lines_per_page", 0) or 0))
        ...
        # Справ у черзі: Нишпорка знає це числом, інакше вважаємо одну.
        cases = max(1, int(getattr(need, "cases", 0) or 1))
        score_need = ScoreNeed(
            ...
            # 🔴 Обсяг НАЙБІЛЬШОГО архіву й межа каналу. Доти плагін ставив лише
            # `data_mb_per_page` — і цим ВИМИКАВ плоску межу каналу воріт (вона
            # знімається, щойно обсяг відомий), не ввівши натомість ворота ціни
            # входу. Тобто на шляху Нишпорки архів 2.25 ГБ на боксі з 21.6
            # Мбіт/с проходив усе: 14 хвилин до першої сторінки, і кожен обрив
            # качання починався заново (spr-160, 22.09.2026).
            # Черга їде одним архівом на справу, тож найбільший archive ≈ увесь
            # обсяг, поділений на справи; коли Нишпорка назвала своє число —
            # беремо його.
            max_archive_mb=max(
                0.0,
                float(getattr(need, "max_archive_mb", 0) or 0)
                or (bytes_in / 1e6 / max(1, cases)),
            ),
            min_net_mbps=max(0.0, float(getattr(need, "min_net_mbps", 0) or 0)),
            cases=cases,
            lines_per_page=lines or LINES_PER_PAGE_ASSUMED,
            lines_assumed=not lines,
            # 🔴🔴 Ціль темпу — і на шляху Нишпорки теж: слабка машина не
            # береться ні наглядачем, ні плагіном. Нишпорка може назвати своє
            # число (`need.target_pph`, 0 — без цілі); не назвала — дефолт.
            target_pph=(DEFAULT_TARGET_PPH if getattr(need, "target_pph", None) is None
                        else max(0.0, float(need.target_pph))),
        )
