...
def ckpt_urls_for(max_hours: float, ckpt_sec: int = CKPT_SEC_DEFAULT) -> int:
    """Скільки посилань треба, щоб чекпоінти дожили до стелі часу (× запас 1.3)."""
    need = int(max_hours * 3600 / max(30, ckpt_sec) * 1.3) + 2
    return max(CKPT_URLS_MIN, need)
...
class BuildOptions:
    ...
    #: 16, а не 64 (06.09.2026): ядра темпу не купують, а `_find_with_patience`
    #: інакше 5 хв чекає 64-ядерну машину, якої на ринку може й не бути.
    prefer_cores: float = 16.0
    wait_min: float = 5.0
    disk_gb: int = 40
    gpu: str = "any"
...
def _case_entry(case_dir: Path, opts: BuildOptions, *, case_key: str,
    ...
    key = f"{opts.prefix.rstrip('/')}/{slug}.tar"
    ...
    if opts.transport == "box":
        ...
        with tempfile.TemporaryDirectory(prefix="htrplan-") as tmp:
            ...
            r2.put(archive, bucket=opts.bucket, prefix=opts.prefix.rstrip("/"), s3=s3)
    else:
        log(f"[план] {slug}: {len(frames)} кадрів (заливку пропущено)")

    n_ckpt = ckpt_urls_for(opts.max_hours)
    ...
    ckpt_prefix = f"ckpt/{slug}/{model_tag}"
    if seed_seg:
        seed_seg_cache(Path(seed_seg), ckpt_prefix, opts, s3=s3, log=log)
    out_dir = (opts.out_root / slug).resolve()
...
def build_plan(case_dirs: list[Path], opts: BuildOptions, *,
    ...
    if assets is not None:
        ...
        # 🔴 Локальний архів заливається ЗАВЖДИ: `--skip-upload` каже «кадри
        # вже в R2», а не «і ассети теж» — 06.09.2026 план зі свіжими ассетами
        # й `--skip-upload` поїхав на бокс із посиланням у порожнечу (HTTP 404
        # на пробі каналу, оренда змарнована). Хто хоче не заливати — дає
        # `--assets-key`.
        if box:
    ...
    plan: dict[str, Any] = {
        ...
        "prefer_min_cores": opts.prefer_cores,
        "wait_for_cores_min": opts.wait_min,
