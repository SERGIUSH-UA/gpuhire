...
def reconcile_with_bill(backend: str, rows: list[tuple[str, float, str]], *,
                        apply: bool = True) -> dict[str, dict[str, float]]:
    """Звірити журнал із рахунком провайдера: `rows` — (handle, $, ISO-час).

    Наявний запис отримує `actual` = рахунок; прогін, якого в журналі немає
    (запущений до журналу, без наглядача), дописується з `reserved` 0. Повертає
    суми по місяцях до й після: `{місяць: {"before": $, "after": $}}`.
    """
    out: dict[str, dict[str, float]] = {}
    with _db() as conn:
        months = {r[0] for r in conn.execute(
            "SELECT DISTINCT month FROM spend WHERE backend = ?", (backend,))}
        months |= {when[:7] for _, _, when in rows if when}
        for m in months:
            out[m] = {"before": _committed(conn, backend, m)}
        if apply:
            conn.execute("BEGIN IMMEDIATE")
            try:
                for handle, usd, when in rows:
                    got = conn.execute("UPDATE spend SET actual = ? WHERE handle = ? AND backend = ?",
                                       (round(float(usd), 4), handle, backend)).rowcount
                    if not got:
                        conn.execute(
                            "INSERT INTO spend (handle, backend, month, ts, reserved, actual, meta)"
                            " VALUES (?, ?, ?, ?, 0, ?, ?)",
                            (handle, backend, when[:7], when, round(float(usd), 4),
                             json.dumps({"source": "vast_bill"})))
                conn.execute("COMMIT")
            except Exception:
                conn.execute("ROLLBACK")
                raise
        for m in out:
            out[m]["after"] = _committed(conn, backend, m) if apply else out[m]["before"]
    return out
