...
def evaluate(probe: dict[str, Any], offer: dict[str, Any], need: Need) -> GateResult:
    ...
    if (need.data_mb_per_page > 0 and measured["net_measured"] and net > 0
            and sizing.usable):
        need_mbps = need.data_mb_per_page * sizing.pages_per_hour / 3600.0 * 8 * NET_HEADROOM
        if net < need_mbps:
            slowdown = need_mbps / net
            sizing = replace(sizing, pages_per_hour=sizing.pages_per_hour / slowdown)
            hours = predict_hours(need.pages, sizing, warm=need.warm)
            cost = predict_cost(need.pages, sizing, offer_dph(offer), warm=need.warm)
    net_why = (f"канал {net:.1f} Мбіт/с не встигає за флотом (треба ~{need_mbps:.1f} на "
    ...
    if need.max_archive_mb > 0 and measured["net_measured"] and net > 0:
        pull_min = need.max_archive_mb * 8 / net / 60.0
        ...
        if pull_min > limit_min:
            return GateResult(
                ...
                f"{net:.1f} Мбіт/с тягнеться {pull_min:.0f} хв при стелі {limit_min:.0f}"
    ...
    per_1000 = 1000.0 * offer_dph(offer) / sizing.pages_per_hour
    base_per_1000 = 1000.0 * offer_dph(offer) / base_pph
