...
def meta_model(out_dir: Path) -> str:
    """Якою моделлю записано текст теки (ім'я файла моделі з `_htr_meta.json`)."""
    try:
        meta = json.loads((Path(out_dir) / "_htr_meta.json").read_text(encoding="utf-8"))
    except (OSError, ValueError):
        return ""
    return pathlib.PurePath(str(meta.get("model") or "")).name if isinstance(meta, dict) else ""


def verify_case(out_dir: Path, *, expected_hint: int | None = None,
                model: str = "") -> Completeness:
    """Звірити забрану теку. `expected_hint` — знаменник із плану, якщо він є.

    `model` — модель плану. 🔴🔴 Текст ІНШОЇ моделі — не «прочитано»: 23.09.2026
    175 тек із латинським декодом Скриби пройшли як `done` для кириличного
    прогону Писарем, бо звірка рахувала будь-які `*.txt`. Реєстр після цього
    вважав справи прочитаними, а догін їх би не взяв.
    """
    out_dir = Path(out_dir)
    texts = _texts(out_dir)
    got = len(texts)
    summary = _summary(out_dir)
    want = pathlib.PurePath(str(model or "")).name
    have = meta_model(out_dir) if want else ""
    if want and have and have != want:
        return Completeness(
            False, int(expected_hint or 0), 0, source="model",
            detail=f"текст у теці записано моделлю {have}, а план — {want}: "
                   f"цією моделлю справу НЕ прочитано",
        )
