...
class CasePlan:
    ...
    #: Куди раннер кладе службовий архів справи (`service.tgz`: лог раннера,
    #: стан, прогрес, логи шардів) і звідки ми його беремо. Той самий шлях, яким
    #: їдуть чекпоінти: presigned PUT/GET у бакеті або склад на самій машині.
    #:
    #: 🔴 Було інакше, і це коштувало заходу. Службові файли забирались по SFTP
    #: пофайлово: `listdir` на КОЖНУ справу черги плюс `get` на КОЖЕН лог
    #: шарда — на 236 справах ~2600 обертів через океан. Забір висів понад 30
    #: хвилин, з'їв усю стелю фази й лишив 237 справ без вердикту — при цілому
    #: результаті, який уже лежав у бакеті. Тут стояв `result_put_url`, який не
    #: писав і не читав НІХТО: готовий слот, що виглядав як робота.
    service_put_url: str = ""
    service_url: str = ""
...
class Plan:
    ...
    assets_url: str
    cases: list[CasePlan]
    #: Ідентифікатор плану і серцебиття бокса в сховищі (PUT — боксу, GET —
    #: дому). Порожні — план старший за серцебиття.
    plan_id: str = ""
    ...
    #: ⚠ З 23.09.2026 НЕ ДІЮТЬ: планка ядер не міняла порядку машин, а після
    #: очікування закінчувалась `settled_for_less`. Вибір іде за `target_pph`.
    #: Поля лишились, щоб старі плани розбирались, а попередження казало про це.
    prefer_min_cores: float = 0.0
    wait_for_cores_min: float = 0.0
    #: 🔴🔴 ЦІЛЬОВИЙ ТЕМП, стор/год. Машина, чий обережний прогноз нижчий, не
    #: береться НІКОЛИ (`offer_score`). 0 — старий режим без цілі (явно).
    target_pph: float = DEFAULT_TARGET_PPH
    #: Скільки хвилин чекати ринку, коли жодна машина не дає цілі. Чекання
    #: безкоштовне (оренди немає); по його кінцю — `market_empty`, а не
    #: згода на слабшу машину.
    ...
    #: Стеля ціни ядро-години, $. `None` = дефолт `offer_score`.
...
_KNOWN_TOP_KEYS = frozenset({
    ...
    "max_usd_per_1000_pages", "min_pages_per_hour", "max_usd_per_core_h", "max_attempts", "max_rents", "gb_per_shard",
    "vram_gb_per_shard", "shards", "catchup_passes", "params",
    "keep_warm_min", "scripts_sha256", "post_fetch", "target_pph", "max_wait_min",
    # 🔴 `transport` і `assets_path` план ЧИТАЄ (нижче, у розборі), але в
    # цьому переліку їх не було — і сторож на кожному заході казав
    # «ключ верхнього рівня невідомий, НЕ ДІЄ» про налаштування, яке
    # насправді діє. 23.09.2026 через це переробили робочий план:
    # вирішили, що транспорт не застосовується. Попередження, яке
    # бреше, дорожче за відсутнє — воно змушує міняти те, що працює.
    "transport", "assets_path",
    "plan_id", "heartbeat_put_url", "heartbeat_url",
})

#: Ключі, які план розуміє всередині справи.
_KNOWN_CASE_KEYS = frozenset({
    "case", "pages_url", "n_pages", "out_dir", "case_key", "case_dir",
    "ckpt_urls", "resume_urls", "service_put_url", "service_url", "params",
    "flatten_out",
...
#: Ключі, які план ще розбирає (старі плани), але які більше нічого не
#: вирішують. Мовчати про них не можна: людина, що виставила `prefer_min_cores`,
#: має знати, що той не діє.
_RETIRED_KEYS = frozenset({"prefer_min_cores", "wait_for_cores_min",
                           "time_value_usd_per_hour"})


def _target_pph(raw: dict[str, Any]) -> float:
    """Ціль темпу: верхній рівень, потім `params` (так її кладе `-p`), потім дефолт.

    🔴 Нуль — законне ЯВНЕ значення («без цілі»), тож перевіряється `is None`,
    а не істинність: `plan_knob` через `if value:` уже раз перетворив явний нуль
    на дефолт (`max_usd_per_core_h=0` не знімав ворота).
    """
    for source in (raw, raw.get("params") or {}):
        value = source.get("target_pph")
        if value is not None:
            return max(0.0, float(value))
    return DEFAULT_TARGET_PPH
...
def check_plan_keys(raw: dict[str, Any]) -> list[str]:
    ...
    """
    warnings: list[str] = []
    for key in sorted(set(raw) & _RETIRED_KEYS):
        if raw.get(key):
            warnings.append(f"`{key}` більше НЕ ДІЄ (з 23.09.2026): вибір машини іде "
                            f"за `target_pph`, а не за планкою ядер чи ціною години чекання")
...
def load_plan(path: str | Path) -> Plan:
    ...
    for i, item in enumerate(cases_raw, 1):
        ...
        cases.append(
            CasePlan(
                ...
                service_put_url=str(item.get("service_put_url") or ""),
                service_url=str(item.get("service_url") or ""),
    ...
    return Plan(
        ...
        heartbeat_put_url=str(raw.get("heartbeat_put_url") or ""),
        heartbeat_url=str(raw.get("heartbeat_url") or ""),
        transport=transport,
        ...
        # `0` тут — ЯВНЕ рішення «без цілі», тож не `or`: відсутній ключ дає
        # дефолт, нуль лишається нулем.
        target_pph=_target_pph(raw),
        max_wait_min=(float(raw["max_wait_min"]) if raw.get("max_wait_min") is not None
                      else 60.0),
        ...
        max_usd_per_core_h=(float(raw["max_usd_per_core_h"])
                            if raw.get("max_usd_per_core_h") is not None else None),
