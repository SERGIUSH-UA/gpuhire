...
class SupervisorState:
    ...
    #: Усі інстанси, орендовані цією сесією: за ними рахунок Vast (`charges`)
    #: дає справжні гроші сесії, навіть коли наглядача вже немає.
    ...
    heartbeat: str = ""
    #: Адреса складу на машині разом із секретом заходу — коли він відкритий
    #: назовні.
    #:
    #: 🔴 Це ЄДИНИЙ спосіб урятувати роботу в режимі `box`, якщо наглядач помер
    #: після читання й до забору. У бакеті на цей випадок лежать присигновані
    #: посилання, записані в ПЛАН; у режимі `box` плану про машину знати нема
    #: звідки — її на той момент ще не існувало. Тому адресу пише сюди той, хто
    #: її дізнався, і `htr fetch-ckpt` читає її звідси.
    #:
    #: ⚠ Містить секрет заходу. Стан лежить у теці даних користувача; секрет
    #: одноразовий і вмирає разом із машиною.
    origin_base: str = ""
