...
def _start_shard(k, shards, base, env, logs_dir, fleet):
    ...
    queue_arg = next((cmd[i + 1] for i, a in enumerate(cmd[:-1]) if a == "--queue"), None)
    if queue_arg:
        _clear_drains(Path(queue_arg).parent, k + 1)
            ...
            for raw in proc.stdout:
                ...
                if line.startswith(PROGRESS_PREFIX):
                    try:
                        ...
                        continue
                    phase = ev.get("phase")
                    if phase in ("page_start", "htr", "case_done", "queue_wait", "queue_done"):
                        # Шард уже йде чергою (модель завантажена). Доти він
                        # не тримає жодної справи — див. `_case_left_by_all`.
                        state["queue_seen"] = True
                    if ev.get("qi") is not None and phase != "case_done":
                        state["qi"] = ev.get("qi")
                    if phase == "htr" and ev.get("qi") is not None and not ev.get("error"):
                        per = fleet.setdefault("qi_pages", {})
                        qi = int(ev.get("qi"))
                        per[qi] = per.get(qi, 0) + 1
                    if phase == "case_done":
                        # 🚂 конвеєр: шард вийшов зі справи — її мета дописана
                        state["qi_done"] = max(int(state.get("qi_done") or 0),
                                               int(ev.get("qi") or 0))
                        # Справу хтось пройшов до кінця — стан флоту, а не
                        # шарда: піднятий на місце мертвого шард цього не знає.
                        fleet.setdefault("qi_left", {})[int(ev.get("qi") or 0)] = True
                    elif phase == "page_start":
...
def _watch_fleet(fleet, out_dir, *, shards, stall_sec, restart_max, base, env, logs_dir, t0,
                 regulator=None, out_dir_of=None):
    ...
    while True:
        ...
        for k in sorted(fleet["shards"]):
            ...
            if rc is not None:
                ...
                # 🚂 У конвеєрі шард живе всю чергу: помер посеред неї — флот
                # тоншав би до кінця заходу. Піднімаємо знову; сторінка, на якій
                # він помер ДВІЧІ, іде в карантин, як і при зависанні (нативна
                # смерть сегментатора не лишає ні винятку, ні рядка в лозі).
                if (out_dir_of is not None and rc not in (0, None)
                        and not state.get("draining")
                        and state.get("revives", 0) < restart_max):
                    page = state.get("inflight")
                    if page:
                        suspects = fleet.setdefault("suspects", {})
                        suspects[page] = suspects.get(page, 0) + 1
                        if suspects[page] >= QUARANTINE_HITS:
                            _quarantine_add(out_dir_of(state), page,
                                            "шард помирав на ній %d рази" % suspects[page])
                            if page not in fleet["quarantine"]:
                                fleet["quarantine"].append(page)
                    revives = state.get("revives", 0) + 1
                    print("[htr-case] ↻ шард %d помер посеред черги (rc=%s) — піднімаю "
                          "(%d/%d)" % (k, rc, revives, restart_max), flush=True)
                    _start_shard(k - 1, denom, base, env, logs_dir, fleet)
                    fleet["shards"][k]["revives"] = revives
                    rcs.pop(k, None)
            ...
            if page != "?" and suspects[page] >= QUARANTINE_HITS:
                # У конвеєрі тека — тієї справи, де шард завис, а не одна на флот.
                qdir = out_dir_of(state) if out_dir_of is not None else out_dir
                _quarantine_add(qdir, page, "вотчдог: %d хв тиші ×%d"
...
#: Сторінки, прочитані на цьому боксі від старту заходу. Монотонний: лише
#: він дає наглядачу темп оренди без знання індексів і фаз. Посправний
#: `pages_done` і `case_index` на переході з конвеєра до великих справ
#: розходились із чергою наглядача — лічильник падав до нуля, і правило
#: «гроші проти сторінок» погасило машину, що читала (spr-80, 24.09.2026).
_BOX_PAGES = [0]
#: Сторінки, що вже мали текст на вході справи (підняті з чекпоінтів), за захід.
_BOX_RESUMED = [0]
...
def _publish(fleet, t0):
    ...
    fleet_out = {
        ...
        # Прочитане на цьому боксі за ВЕСЬ захід: не скидається ні між справами,
        # ні на переході з конвеєра до великих справ (див. `_BOX_PAGES`).
        "box_pages_done": _BOX_PAGES[0],
        "box_pages_resumed": _BOX_RESUMED[0],
        "pages_failed": failed,
    ...
    for key in ("missing", "missing_count", "complete", "catchup_rounds", "result_url",
                "case_index", "cases_total", "final_case", "ckpt", "pipeline"):
        if key in fleet:
...
#: Presigned PUT серцебиття в сховищі (з параметрів плану); порожній — немає.
_BEAT_URL = [""]
#: Як часто серцебиття їде в сховище, с.
REMOTE_BEAT_SEC = 300
_BOX_STARTED = time.time()


def _beat_to_store(final=False):
    """Серцебиття в сховище: лог раннера, стан, прогрес і `beat.json`.

    Один об'єкт, що перезаписується: без наглядача з нього видно, що бокс
    робив, на чому зупинився і коли востаннє жив — тобто скільки коштував.
    """
    url = _BEAT_URL[0]
    if not url:
        return False
    root = PROGRESS_PATH.parent
    tmp = Path("/tmp/htrcase/_beat.tgz")
    try:
        tmp.parent.mkdir(parents=True, exist_ok=True)
        beat = {"ts": _utc_iso(), "t": time.time(), "started": _BOX_STARTED,
                "final": bool(final),
                "container": os.environ.get("CONTAINER_ID") or os.environ.get("VAST_CONTAINERLABEL") or "",
                "box_pages_done": _BOX_PAGES[0], "box_pages_resumed": _BOX_RESUMED[0],
                "results": list(_QUEUE_RESULTS)}
        with tarfile.open(tmp, "w:gz", compresslevel=6) as tf:
            data = json.dumps(beat, ensure_ascii=False).encode("utf-8")
            info = tarfile.TarInfo("beat.json")
            info.size = len(data)
        ...
        rc = subprocess.call(["curl", "-fsS", "--retry", "2",
                              "--max-time", str(_upload_ceiling(tmp.stat().st_size)),
                              "-X", "PUT", "--upload-file", str(tmp), url],
...
def _heartbeat(period=10):
    ...
    while not _HEARTBEAT_STOP.wait(period):
        if _BEAT_URL[0] and time.time() - last_remote >= REMOTE_BEAT_SEC:
...
#: Службові файли заходу: лог раннера, стан, прогрес. Дрібні, але саме через
#: них забір висів понад 30 хвилин — пофайловим SFTP на КОЖНУ справу черги.
SERVICE_FILES = ("_runner.log", "_status.json", "_progress.json")

#: Підлога стелі часу на одну віддачу, с, і песимістичний темп, МБ/с.
#:
#: 🔴 `curl` без `--max-time` висне НАВІЧНО, і саме це ми вже ловили в
#: наглядача. У раннера це дорожче: віддача чекпоінта стоїть у циклі роботи,
#: тож завислий `curl` спиняє не забір, а ЧИТАННЯ — бокс тарифікується,
#: прогрес не рухається, і зовні це виглядає як «працює».
#:
#: Темп навмисно втричі-вчотирнадцятеро нижчий за виміряний (HTTP до бакета
#: дає одиниці-десятки МБ/с): це стеля АВАРІЇ, а не очікуваний час. Вона
#: мусить ловити мертвий сокет, а не карати повільний, але живий канал.
UPLOAD_MIN_SEC = 120
UPLOAD_MB_PER_SEC = 0.5


def _upload_ceiling(size_bytes):
    """Скільки секунд щонайбільше давати одній віддачі такого розміру."""
    return int(max(UPLOAD_MIN_SEC, (size_bytes or 0) / 1e6 / UPLOAD_MB_PER_SEC))


def _service_to_url(url, case_out, root=None):
    """Службові файли ОДНИМ архівом через той самий presigned PUT.

    🔴 Навіщо. Доти дім забирав їх по SFTP, і на черзі це вироджувалось у
    `listdir` на кожну справу плюс `get` на кожен лог шарда — ~2600 операцій на
    236 справах, понад 30 хвилин оплаченої оренди, після чого фаза забору
    здавалась. Тіло результату при цьому давно лежало в сховищі.

    Тут той самий шлях, що й у чекпоінтів: один tar, один `curl -X PUT`. Дім
    бере його одним GET.

    Best-effort і за побудовою: діагностика не варта того, щоб через неї падала
    справа. Чого немає — того немає; помилку друкуємо й повертаємо 0.
    """
    if not url:
        return 0
    # Той самий корінь, у якому лежить канал прогресу, — другого літерала тут
    # бути не має: розійдуться вони тихо.
    root = root or str(PROGRESS_PATH.parent)
    try:
        tmp = Path("/tmp/htrcase/service.tgz")
        tmp.parent.mkdir(parents=True, exist_ok=True)
        added = 0
        with tarfile.open(tmp, "w:gz", compresslevel=1) as tf:
            for name in SERVICE_FILES:
                ...
                if src.is_file():
                    tf.add(src, arcname=name)
                    added += 1
            logs = Path(case_out) / "logs" if case_out else None
            if logs and logs.is_dir():
                for log in sorted(logs.glob("*.log")):
                    tf.add(log, arcname="logs/" + log.name)
                    added += 1
        if not added:
            tmp.unlink(missing_ok=True)
            return 0
        ...
        rc = subprocess.call(
            ["curl", "-fsS", "--retry", "2",
             "--max-time", str(_upload_ceiling(size)),
             "-X", "PUT", "--upload-file", str(tmp), url])
        tmp.unlink(missing_ok=True)
        if rc != 0:
            print("[htr-case] \u26a0 службовий архів не залився (curl rc=%s) \u2014 "
                  "діагностика лишиться на боксі" % rc, flush=True)
            return 0
        print("[htr-case] службовий архів: %d файл(ів), %.1f КБ \u2192 сховище"
              % (added, size / 1024.0), flush=True)
        return added
    except Exception as exc:
        print("[htr-case] \u26a0 службовий архів не склався: %r" % (exc,), flush=True)
        return 0


#: Прохання наглядача «чекпоінт ЗАРАЗ» і відповідь раннера. Наглядач перед
#: забором ставить прапорець одним SSH-викликом і чекає відповіді: текст іде в
#: сховище HTTP-заливкою, а не пофайловим SFTP через океан.
CKPT_NOW = Path("/tmp/htrcase/ckpt_now")
CKPT_DONE = Path("/tmp/htrcase/ckpt_done")
#: Хто відповідає на прохання: pid процесу з живим циклом чекпоінтів.
CKPT_PID = Path("/tmp/htrcase/ckpt.pid")
#: Як часто цикл перевіряє прохання, с.
CKPT_POLL_SEC = 2.0


def _checkpoint_loop(stop, every, push):
    """Раунд чекпоінта кожні `every` с — або одразу, щойно попросив наглядач."""
    try:
        CKPT_PID.parent.mkdir(parents=True, exist_ok=True)
        CKPT_PID.write_text(str(os.getpid()))
    except OSError:
        pass
    last = time.time()
    while not stop.wait(CKPT_POLL_SEC):
        asked = CKPT_NOW.exists()
        if not asked and time.time() - last < every:
            continue
        try:
            push()
        except Exception as exc:
        ...
        last = time.time()
        if asked:
            CKPT_NOW.unlink(missing_ok=True)
            try:
                CKPT_DONE.write_text(_utc_iso())
            except OSError:
                pass
            print("[htr-case] чекпоінт на прохання наглядача — у сховищі", flush=True)


class _CkptSink:
    """Чекпоінти справи в presigned PUT-слоти — такі, що НЕ вичерпуються.

    🔴🔴 Слоти нумеровані й кінечні, а раунд чекпоінта — кожні `checkpoint_sec`.
    Коли слоти закінчувались, чекпоінти мовчки припинялись до кінця заходу,
    фінальний теж. 23.09.2026 книга ЦДІАК spr-68: план дав 16 слотів на 1 308
    сторінок (одна точка на ~100 сторінок), раннер спалив їх за перші ~32 хв,
    у сховищі лишилось 719 сторінок, решта 589 — лише на диску бокса, який
    знищили. Тому ОСТАННІЙ слот ковзний: presigned PUT на той самий ключ
    перезаписує об'єкт, і кожен раунд після вичерпання перезаливає туди ВСЕ, що
    з'явилось після передостаннього слота. Розпакування по порядку (як і
    раніше) дає повний стан, бо ковзний архів — надмножина свого хвоста.

    Потокобезпечний: у конвеєрі пише і періодичний потік, і закриття справи.
    """

    #: Скільки провалених заливок поспіль означає «точки відновлення немає»
    #: (протухлі посилання, мертвий канал) — це бачить наглядач.
    FAIL_STREAK_STUCK = 3

    def __init__(self, work, urls, start=0, label=""):
        self.work = Path(work)
        self.urls = list(urls or [])
        self.label = label
        self.rounds = int(start or 0)   # зайняті слоти, разом із попередньою орендою
        self.seen = {}
        self.base = None                # `seen` до першого запису в ковзний слот
        ...
        self.fail_streak = 0
        self.last_tail = False

    def push(self):
        """Один раунд. Повертає, скільки НОВИХ файлів тепер у сховищі (0 — нічого)."""
        with self.lock:
            if not self.urls:
                return 0
            fresh = _new_files(self.work, self.seen)
            if not fresh:
                return 0
            n = len(self.urls)
            if self.rounds + 1 < n:
                index, files = self.rounds + 1, fresh
            else:
                if self.base is None:
                    self.base = dict(self.seen)
                index, files = n, _new_files(self.work, self.base)
            t = time.time()
            ok, size = _put_archive(files, self.urls[index - 1])
            self.last_sec = round(time.time() - t, 1)
            if not ok:
                self.fail_streak += 1
                print("[htr-case] ⚠ чекпоінт %s№%d не залився — файли лишаються в "
                      "позначених, спробуємо наступним раундом"
                      % (self.label + " " if self.label else "", index), flush=True)
                return 0
            self.fail_streak = 0
            for _, rel, sz in files:
                self.seen[rel] = sz
            self.last_tail = index == n and self.rounds >= n
            if self.last_tail:
                self.tail_rewrites += 1
            self.rounds = max(self.rounds, index)
            self.ok += 1
            self.files += len(fresh)
            self.last_ok = _utc_iso()
            print("[htr-case] чекпоінт %s№%d%s → хмара: %d нових, %d у архіві, %.1f МБ за %.1f с"
                  % (self.label + " " if self.label else "", index,
                     " (ковзний)" if self.last_tail else "", len(fresh), len(files),
                     size / 1e6, self.last_sec), flush=True)
            return len(fresh)

    @property
    def stuck(self):
        return self.fail_streak >= self.FAIL_STREAK_STUCK


def _put_archive(files, url):
    """Запакувати `files` і залити одним PUT. (вдалось?, байтів)."""
    import tempfile

    Path("/tmp/htrcase").mkdir(parents=True, exist_ok=True)
    # Ім'я унікальне: у конвеєрі кілька справ пишуть чекпоінти одночасно.
    fd, name = tempfile.mkstemp(prefix="ckpt_", suffix=".tgz", dir="/tmp/htrcase")
    os.close(fd)
    tmp = Path(name)
    try:
        with tarfile.open(tmp, "w:gz", compresslevel=1) as tf:
            for src, rel, _ in files:
                tf.add(src, arcname=rel)
        size = tmp.stat().st_size
        # 🔴 Стеля рахується від РОЗМІРУ: `curl` без `--max-time` на завислому
        # сокеті стоїть навічно.
        rc = subprocess.call(["curl", "-fsS", "--retry", "2",
                              "--max-time", str(_upload_ceiling(size)),
                              "-X", "PUT", "--upload-file", str(tmp), url])
        return rc == 0, size
    except OSError as exc:
        print("[htr-case] ⚠ архів чекпоінта не зібрався: %r" % (exc,), flush=True)
        return False, 0
    finally:
        tmp.unlink(missing_ok=True)


def _ckpt_health(health, sinks):
    """Здоров'я чекпоінтів для наглядача — сумою по справах."""
    sinks = [k for k in sinks if k is not None]
    health["ok"] = sum(k.ok for k in sinks)
    health["files"] = sum(k.files for k in sinks)
    oks = [k.last_ok for k in sinks if k.last_ok]
    health["last_ok"] = max(oks) if oks else None
    health["tail_rewrites"] = sum(k.tail_rewrites for k in sinks)
    health["exhausted"] = any(k.stuck for k in sinks)
    return health
...
def main(params):
    ...
    """
    _BEAT_URL[0] = str((params or {}).get("heartbeat_put_url") or "")
...
#: Скільки з'єднань відкривати на один архів кадрів, коли один потік не тягне.
#:
#: 🔴🔴 Навіщо взагалі: ОДИН потік до R2 на далекому плечі стає причиною
#: ВІДМОВ, а не просто повільності. Бокс міряє собі 166-424 Мбіт/с, а реальне
#: качання давало 913 МБ за 417 с (2.2 МБ/с ≈ 17.5 Мбіт/с) — удесятеро нижче.
#: 22.09.2026 на цьому впали ДВІ машини поспіль (`spr-1067`: rc=28, далі «бокс
#: завис на качанні архіву»), 14.09 на irnbuv1 q23 так загинуло 8 томів із 23.
#:
#: 🔴🔴 І чому це НЕ вмикається завжди. Заміряно 22.09.2026 на здоровому
#: каналі, чергуючи способи на різних зрізах того самого архіву R2:
#:
#:     1 з'єднання  — 20.07 МБ/с
#:     4 з'єднання  — 14.63 МБ/с
#:     8 з'єднань   — 15.95 МБ/с
#:
#: Там, де один потік уже вибирає канал, дроблення коштує ЧВЕРТЬ швидкості:
#: вісім TLS-рукостискань, вісім підписів і конкуренція за ту саму трубу. Тому
#: паралель не стан за замовчуванням, а РЯТУНОК: качання починається одним
#: потоком і переходить у кілька лише тоді, коли виміряний темп цього заходу
#: виявився поганим (`DOWNLOAD_ESCALATE_BPS`). Здоровий маршрут при цьому
#: працює рівно як раніше.
DOWNLOAD_PARTS = 8
#: Скільки дивитись на один потік, перш ніж судити про його темп.
DOWNLOAD_ESCALATE_SEC = 45
#: Нижче цього темпу один потік визнається безнадійним і решта файла береться
#: кількома з'єднаннями. 4 МБ/с чисто розділяє здоровий маршрут (20+ МБ/с
#: заміряно) і хворий (0.3-2.2 МБ/с, Таїланд).
DOWNLOAD_ESCALATE_BPS = 4 * 2 ** 20
#: Внутрішній код: «один потік безнадійний, качай решту частинами».
_ESCALATE = -3
#: Дрібний архів дробити немає сенсу: накладні на пробу з'їдять виграш.
DOWNLOAD_PARALLEL_MIN_BYTES = 32 * 2 ** 20
#: Найменша частина. Двадцять з'єднань по 3 МБ — це не швидше, а лише 20 шансів
#: обірватись.
DOWNLOAD_MIN_PART_BYTES = 8 * 2 ** 20
#: Запас диска для склейки: частини + готовий файл живуть поруч.
DOWNLOAD_PARALLEL_DISK_FACTOR = 2.2


def _part_paths(local, parts):
    return [local.parent / ("%s.part%02d" % (local.name, i)) for i in range(parts)]


def _drop_parts(paths):
    for p in paths:
        try:
            p.unlink()
        except OSError:
...
# 🔴🔴 Привезене переживає обрив. 24.09.2026, RTX 3090 (Мічиган): маршрут до R2
# просів з 26 МБ/с до 0-4 МБ/с, і том spr-102 (5.6 ГБ) шість разів поспіль
# качався З НУЛЯ. Одна з восьми частин падала нижче підлоги — і `_drop_parts`
# стирав усі вісім, зокрема докачані; після цього продовження вимикалось, а
# невдале передзавантаження стирало свій огризок, і звичайне качання того самого
# тому починало знову. ~2.3 ГБ викинуто, ~30 хв оплаченого простою, бокс
# погашено як удвічі дорожчий.
#
# Тепер поруч із файлом лежить реєстр `<ім'я>.dl.json`: повний розмір об'єкта,
# його ETag і розкладка частин. Доки реєстр збігається, частини докачуються з
# того байта, на якому стали. Не збігся (інший об'єкт, інша розкладка) — усе
# стирається, і качання йде з нуля, як раніше.

#: ETag об'єкта з останньої проби діапазону, за шляхом файла.
_OBJECT_TAG = {}


def _ledger_path(local):
    return local.parent / ("%s.dl.json" % local.name)


def _read_ledger(local):
    try:
        return json.loads(_ledger_path(local).read_text(encoding="utf-8"))
    except (OSError, ValueError):
        return None


def _write_ledger(local, **fields):
    led = _read_ledger(local) or {}
    led.update(fields)
    try:
        _ledger_path(local).write_text(json.dumps(led), encoding="utf-8")
    except OSError:
        pass
...
def _parts_bytes(local):
    """Скільки привезено в частинах, що ще не склеєні."""
    return sum(_file_size(p) for p in _part_paths(local, DOWNLOAD_PARTS))


def _have_bytes(local):
    return _file_size(local) + _parts_bytes(local)


def _drop_download(local):
    """Стерти все качання: файл, частини, реєстр."""
    _drop_parts([local, *_part_paths(local, DOWNLOAD_PARTS), _ledger_path(local)])


def _adopt_leftovers(local, total):
    """Привезене попереднім викликом — лишити, якщо це той самий об'єкт.

    Той самий — це той самий повний розмір і той самий непорожній ETag. Усе
    інше (реєстру немає, ETag не прийшов, об'єкт змінився) стирається: чужі
    байти в архіві кадрів — найдорожчий клас помилок.
    """
    leftovers = local.exists() or _parts_bytes(local) > 0 or _ledger_path(local).exists()
    if not leftovers:
        return
    led = _read_ledger(local) or {}
    tag = _OBJECT_TAG.get(str(local), "")
    if total and tag and led.get("total") == total and led.get("etag") == tag:
        have = _have_bytes(local)
        if have > 0 and _file_size(local) < total:
            print("[htr-case] ⏬ %s: продовжую з %.0f МБ, привезених раніше"
                  % (local.name, have / 1e6), flush=True)
            return
    _drop_download(local)


def _ranged_plan(url, local):
    """Повний розмір об'єкта — або 0, якщо дробити на діапазони не можна.

    🔴 Проба робиться ЗАПИТОМ GET із діапазоном `0-0`, а не `HEAD`: підпис
    presigned-посилання R2 прив'язаний до методу, тож `HEAD` на ньому дає 403,
    і сервер, який чудово вміє діапазони, виглядав би як такий, що не вміє.

    🔴 Приймач — саме `Content-Range` у відповіді. Сервер, який діапазон
    ІГНОРУЄ, віддає 200 і все тіло; якби ми повірили йому на слово, кожна з
    восьми частин привезла б увесь архів, а склейка дала б сміття.
    """
    try:
        out = subprocess.run(["curl", "-fsS", "--max-time", "60", "-r", "0-0",
                              "-D", "-", "-o", os.devnull, url],
                             stdout=subprocess.PIPE, stderr=subprocess.DEVNULL,
                             timeout=90).stdout.decode("utf-8", "replace")
    except Exception as exc:
        print("[htr-case] ⏬ проба діапазону не вдалась (%r) — качаю одним потоком" % (exc,),
              flush=True)
        return 0
    total = 0
    tag = ""
    for line in out.splitlines():
        ...
        if low.startswith("content-range:") and "/" in line and not total:
            tail = line.rsplit("/", 1)[-1].strip()
            if tail.isdigit():
                total = int(tail)
    _OBJECT_TAG[str(local)] = tag
    if total < DOWNLOAD_PARALLEL_MIN_BYTES:
        return 0
    free = _disk_free_bytes(local.parent)
    if free is not None and free < total * DOWNLOAD_PARALLEL_DISK_FACTOR:
        print("[htr-case] ⏬ на склейку не вистачить диска (%.1f ГБ вільно) — одним потоком"
              % (free / 2 ** 30,), flush=True)
        return 0
    return total


def _stop(proc):
    """Погасити ОДИН curl. Рівно як було до поділу шляхів: `kill` + `wait`.

    🔴 Без `poll()`: однопотокову гілку тести описують мінімальним двійником
    `Popen`, у якого є лише `wait` і `kill`, і це правильно — саме ці два
    виклики вона й робила весь час у продакшені.
    """
    proc.kill()
    try:
        proc.wait(timeout=60)
...
def _kill_all(procs):
    """Погасити флот діапазонних з'єднань (там `Popen` справжній)."""
    for p in procs:
        if p.poll() is None:
            p.kill()
    for p in procs:
...
def _watch(procs, sizes_of, name, heartbeat, min_free_bytes, attempt, where):
    """Нагляд за ПАРАЛЕЛЬНИМ качанням: серцебиття, застій, обрив по диску.

    Повертає список кодів повернення або `None` при обриві через диск.

    🔴 Однопотокова гілка свідомо має власний цикл на `proc.wait(timeout=…)`:
    вона працює в продакшені з самого початку, і її форму стережуть тести.
    Тут, де процесів вісім, `poll()` доречний — але тільки тут.
    """
    last = -1
    while any(p.poll() is None for p in procs):
        time.sleep(DOWNLOAD_HEARTBEAT_SEC)
        size = sizes_of()
        if heartbeat:
            _set_phase("downloading", download=name, bytes=size, attempt=attempt,
                       streams=len(procs), stalled=(size == last))
        if min_free_bytes:
            free = _disk_free_bytes(where)
...
def _download_ranged(url, local, name, total, parts, floor, heartbeat, min_free_bytes,
                     attempt, begin=0):
    """Архів кількома з'єднаннями, кожне — свій діапазон байтів.

    ``begin`` — з якого байта качати: те, що один потік уже встиг привезти,
    лишається на диску й дописується, а не качається вдруге.

    Підлога швидкості ділиться на число частин: вимагати мегабайт за секунду
    від КОЖНОГО з восьми з'єднань означало б обривати цілком здорове качання.

    🔴🔴 Обрив частини НЕ стирає інших. Частини тієї самої розкладки (реєстр
    `.dl.json`: `begin` і число частин) живуть між спробами, і кожна докачується
    з байта, на якому стала: curl бере `-r <старт+є>-<кінець>` і дописує в кінець
    файла. Тому в curl тут немає `--retry`: власний повтор curl почав би діапазон
    наново й дописав би ті самі байти вдруге. Повтор робить цикл спроб вище.
    """
    paths = _part_paths(local, parts)
    led = _read_ledger(local) or {}
    if led.get("total") != total or led.get("begin") != begin or led.get("n") != parts:
        _drop_parts(_part_paths(local, DOWNLOAD_PARTS))
        _write_ledger(local, total=total, etag=_OBJECT_TAG.get(str(local), ""),
                      begin=begin, n=parts)
    ...
    for i, path in enumerate(paths):
        ...
        span = end - start + 1
        spans.append(span)
        have = _file_size(path)
        if have > span:
            # Довша за свій діапазон — битий стан, лише з нуля.
            _drop_parts([path])
            have = 0
        if have == span:
            continue
        ...
        procs.append(subprocess.Popen(
            ["curl", "-fsSL", "--max-time", "7200",
             "--speed-limit", str(max(16384, floor // parts)), "--speed-time", "60",
             "-r", "%d-%d" % (start + have, end), url], stdout=sink))

    def _size():
        return sum(_file_size(p) for p in paths)

    try:
        rcs = _watch(procs, _size, name, heartbeat, min_free_bytes, attempt, local.parent)
    finally:
        for sink in sinks:
            sink.close()
    if rcs is None:
    ...
    if bad:
        # Частини лишаються: наступна спроба докачає лише те, чого бракує.
        return bad[0]
    # 🔴 Звіряємо ДОВЖИНУ кожної частини. Сервер, що мовчки віддав не той
    # діапазон, інакше склеївся б у файл правильного вигляду й неправильного
    # вмісту — а це найдорожчий клас помилок: tar розпакується, кадри будуть
    # чужі, і знаменник повноти цього не побачить.
    for path, want in zip(paths, spans):
        got = _file_size(path) if path.exists() else -1
        if got != want:
            print("[htr-case] ⚠ частина %s: %s Б замість %d — склейку скасовано"
                  % (path.name, got, want), flush=True)
            _drop_parts(paths)
            return 1
    try:
        with local.open("ab" if begin else "wb") as out:
            for path in paths:
                with path.open("rb") as src:
                    shutil.copyfileobj(src, out, 4 * 2 ** 20)
                path.unlink()
    except OSError as exc:
        print("[htr-case] ⚠ склейка %s не вдалась: %r" % (name, exc), flush=True)
        _drop_download(local)
        return 1
    _write_ledger(local, begin=None, n=None)
    return 0


def _download_stream(url, local, name, floor, heartbeat, min_free_bytes, attempt, total=0):
    """Одне з'єднання — основний шлях; при поганому темпі просить ескалацію.

    🔴 Вирок про темп виноситься за ВИМІРЯНИМ качанням, а не за заявою боксу
    про свій канал: `inet_down` в офері й проба при оренді на маршруті до R2 не
    справджуються (заявлено 166-424 Мбіт/с, фактично 2.2 МБ/с). Привезене до
    вироку не викидається — його дописує `_download_ranged`.

    🔴 Чекання лишилось на `proc.wait(timeout=…)`, а не на `poll()`, хоч у
    паралельній гілці зручніший другий. Це єдиний шлях, який працював у
    продакшені весь час, і `tests/test_stall_between_cases_and_wasted_rents.py`
    стереже саме його форму: один виклик `wait` на такт серцебиття. Перехід на
    `poll()` тут нічого не давав і мовчки міняв кількість тактів.
    """
    proc = subprocess.Popen(["curl", "-fsSL", "--retry", "3", "--max-time", "7200",
                             "--speed-limit", str(floor), "--speed-time", "60",
                             "-o", str(local), url])
    last = -1
    started = time.time()
    while True:
        try:
            ...
            if (total and elapsed >= DOWNLOAD_ESCALATE_SEC
                    and size / max(0.001, elapsed) < DOWNLOAD_ESCALATE_BPS
                    and (total - size) >= DOWNLOAD_PARALLEL_MIN_BYTES):
                _stop(proc)
                return _ESCALATE
            if min_free_bytes:
                free = _disk_free_bytes(local.parent)
...
def _download_with_heartbeat(url, local, name, tries=DOWNLOAD_TRIES, heartbeat=True,
    ...
    Обрив НЕ скидає привезеного: повтор продовжує з наявного байта діапазонними
    з'єднаннями, а частини, що вже докачались, лишаються (див. `_adopt_leftovers`).
    З нуля качання починається лише тоді, коли діапазонний шлях за весь виклик
    не привіз жодного байта, — тоді винен він, і лишається перевірений один потік.
    rc curl повертається як є.

    Качання ПОЧИНАЄТЬСЯ одним потоком. Якщо за `DOWNLOAD_ESCALATE_SEC` темп
    не дотягнув до `DOWNLOAD_ESCALATE_BPS`, решта файла береться кількома
    з'єднаннями (див. `DOWNLOAD_PARTS`) — а привезене дописується, не
    качається вдруге. Коли сервер не вміє діапазонів або не вистачає диска на
    склейку, лишається один потік, як було.

    Невдале качання лишає привезене на диску разом із реєстром — його підхопить
    наступний виклик для того самого об'єкта (повтор тому в кінці черги, або
    звичайне качання після невдалого передзавантаження).
    ...
    """
    rc = 1
    # Число частин рахується не тут, а в момент ескалації: до неї вже щось
    # привезено, і дробити треба ЗАЛИШОК, а не повний розмір.
    total = _ranged_plan(url, local)
    _adopt_leftovers(local, total)
    if total:
        _write_ledger(local, total=total, etag=_OBJECT_TAG.get(str(local), ""))
    ranged_worked = False     # діапазонний шлях хоч раз привіз байти

    def _ranged(attempt, floor):
        """Докачати решту частинами; (rc, чи привезено бодай щось)."""
        got = _file_size(local)
        left = total - got
        led = _read_ledger(local) or {}
        n = (led["n"] if led.get("begin") == got and led.get("n")
             else max(2, min(DOWNLOAD_PARTS, left // DOWNLOAD_MIN_PART_BYTES)))
        before = _have_bytes(local)
        rc = _download_ranged(url, local, name, total, int(n), floor,
                              heartbeat, min_free_bytes, attempt, begin=got)
        return rc, rc == 0 or _have_bytes(local) > before

    for attempt in range(1, tries + 1):
        floor = _download_floor(attempt)
        t_attempt = time.time()
        resumed = bool(total) and _have_bytes(local) > 0 and _file_size(local) < total
        if resumed:
            print("[htr-case] ⏬ %s: продовжую з %.0f МБ з %.0f МБ"
                  % (name, _have_bytes(local) / 1e6, total / 1e6), flush=True)
            rc, moved = _ranged(attempt, floor)
            ranged_worked = ranged_worked or moved
        else:
            _drop_parts(_part_paths(local, DOWNLOAD_PARTS))
            rc = _download_stream(url, local, name, floor, heartbeat, min_free_bytes,
                                  attempt, total=total)
            if rc == _ESCALATE:
                got = _file_size(local)
                left = total - got
                n = max(2, min(DOWNLOAD_PARTS, left // DOWNLOAD_MIN_PART_BYTES))
                print("[htr-case] ⏬ %s: один потік дав %.2f МБ/с — решту (%.0f МБ) беру "
                      "в %d з'єднань" % (name, got / 1e6 / max(0.001, time.time() - t_attempt),
                                         left / 1e6, n), flush=True)
                rc, moved = _ranged(attempt, floor)
                ranged_worked = ranged_worked or moved
                if rc not in (0, -2) and not moved:
                    # 🔴 Паралель не привезла НІЧОГО — річ у маршруті, а не в
                    # числі з'єднань. Далі один потік, як до ескалації.
                    print("[htr-case] ⚠ паралельне качання %s не вдалось (rc=%s) — "
                          "далі одним потоком" % (name, rc), flush=True)
                    total = 0
        if rc == -2:
            _drop_download(local)     # диску бракує — привезене не тримаємо
            return -2
        if rc == 0:
            _drop_parts([_ledger_path(local)])
            try:
                size = local.stat().st_size
                if size >= 8 * 2 ** 20:    # малий файл швидкості каналу не показує
                    sec = max(0.001, time.time() - t_attempt)
                    _DOWNLOAD_SEEN_BPS.append(size / sec)
                    print("[htr-case] завантажено %s: %.0f МБ за %.0f с (%.1f МБ/с)"
                          % (name, size / 1e6, sec, size / 1e6 / sec), flush=True)
            except OSError:
                pass
            return 0
        have = _have_bytes(local)
        print("[htr-case] ⚠ качання %s: curl rc=%s (спроба %d/%d, привезено %.0f МБ)"
              % (name, rc, attempt, tries, have / 1e6), flush=True)
        # 🔴🔴 ПРОДОВЖУЄМО, а не качаємо з нуля. `rc=28` означає, що темп упав
        # нижче підлоги на 60 с, — тобто обрив коштує рівно тієї хвилини, а не
        # всього привезеного. 22.09.2026, spr-160: архів 2.25 ГБ на боксі з
        # ВИМІРЯНИМИ 21.6 Мбіт/с (в офері обіцяли 1821) тягнеться 14 хвилин, і
        # кожен повтор з нуля з'їдав їх заново — три спроби по 14 хв оплаченої
        # машини, після чого справа однаково падала.
        #
        # Умова безпеки одна: файл на диску мусить бути ПРЕФІКСОМ об'єкта, а
        # кожна частина — префіксом свого діапазону. Так і є: один потік пише
        # послідовно, частини дописуються в кінець, склейка йде строго по черзі.
        if not resumed and total and have > 0 and _file_size(local) < total \
                and attempt < tries:
            print("[htr-case] ⏬ %s: продовжую з %.0f МБ, лишилось %.0f МБ"
                  % (name, have / 1e6, (total - have) / 1e6), flush=True)
            rc, moved = _ranged(attempt + 1, _download_floor(attempt + 1))
            ranged_worked = ranged_worked or moved
            if rc == -2:
                _drop_download(local)
                return -2
            if rc == 0:
                _drop_parts([_ledger_path(local)])
                print("[htr-case] завантажено %s: %.0f МБ (з продовженням)"
                      % (name, local.stat().st_size / 1e6), flush=True)
                return 0
            print("[htr-case] ⚠ продовження %s не вдалось (rc=%s)" % (name, rc), flush=True)
        if not (total and ranged_worked):
            # Діапазонний шлях за весь виклик не привіз нічого — лишається
            # перевірений: наступна спроба одним потоком і з нуля.
            print("[htr-case] ⏬ %s: наступна спроба з нуля" % name, flush=True)
            _drop_download(local)
        if attempt < tries:
...
class _Prefetch:
    ...
    def take(self, url, resume_to=None):
        """Готовий файл (дочекавшись, якщо ще качається) або None — качати звичайно.

        ``resume_to`` — куди качатиме звичайний шлях. Невдале передзавантаження
        переносить туди привезене разом із частинами й реєстром, і звичайне
        качання продовжує, а не починає з нуля (`_adopt_leftovers` перевірить,
        що це той самий об'єкт).
        """
        ...
        if resume_to is not None and self.rc != -2 and _ledger_path(self.local).exists():
            dst = Path(resume_to)
            try:
                dst.parent.mkdir(parents=True, exist_ok=True)
                _drop_download(dst)
                pairs = [(self.local, dst), (_ledger_path(self.local), _ledger_path(dst))]
                pairs += list(zip(_part_paths(self.local, DOWNLOAD_PARTS),
                                  _part_paths(dst, DOWNLOAD_PARTS)))
                for src, to in pairs:
                    if src.exists():
                        shutil.move(str(src), str(to))
                tag = _OBJECT_TAG.get(str(self.local))
                if tag is not None:
                    _OBJECT_TAG[str(dst)] = tag
                return None
            except OSError as exc:
                print("[htr-case] ⚠ привезене передзавантаженням не перенеслось: %r"
                      % (exc,), flush=True)
        _drop_download(self.local)
        return None
...
def _resume_from_urls(work, resume_urls):
    """Розпакувати в `work` чекпоінти попередніх оренд. (скільки, останній номер).

    Серія чекпоінтів іде без дірок, тож три 404 поспіль — кінець серії.
    """
    got = 0
    ...
    if resume_urls:
        print("[htr-case] з хмари підхоплено %d/%d архів(ів) стану"
              % (got, len(resume_urls)), flush=True)
    return got, last_present


# ── 🚂 конвеєр черги ─────────────────────────────────────────────────────────
# 🔴🔴 Навіщо. Черга читалась справа за справою, і кожна справа платила свій
# повний цикл: чекати кадри, розпакувати, запитати старі чекпоінти, підняти
# шарди (моделі — 7 с на кожен), дочитати хвіст, по черзі залити чекпоінт,
# підсумок і службовий архів. 23.09.2026, 194 справи по ~15 сторінок: справа
# займала ~41 с, з яких читання — ~12. Тобто навіть найкраща машина ринку
# давала на такій черзі ~2 000 стор/год.
#
# Тут шарди стартують ОДИН раз і йдуть по файлу черги (`htr_case_run.py
# --queue`), підготовка наступних справ і закриття попередніх ідуть поруч із
# читанням, а не між справами. Великі справи й повтори неповних лишаються
# звичайному шляху: там накладні розмазані по сотнях сторінок, а догін і
# регулятор на справу вже вивірені.

#: Справи, більші за це, йдуть звичайним шляхом (див. вище).
PIPELINE_MAX_PAGES = 400
#: Запас НЕПРОЧИТАНИХ сторінок, який підготовка тримає перед шардами: не менше
#: `PIPELINE_BUFFER_MIN` і по `PIPELINE_BUFFER_PER_SHARD` на живий шард.
#: 🔴 Доти вікно міряло кількість НЕЗАКРИТИХ справ (4). Одна повільна сторінка
#: чи шард, що вантажить модель, не давали закрити голову черги, вікно
#: заповнювалось, і решта флоту стояла: приймач 23.09.2026 — дві паузи, 18 хв і
#: 3 хв, із 47 хв заходу (темп 3 658 замість 7–8.7 тис. між паузами).
PIPELINE_BUFFER_MIN = 60
PIPELINE_BUFFER_PER_SHARD = 12
#: Запобіжник диска: більше стільки розпакованих справ одночасно не буває.
PIPELINE_OPEN_MAX = 32
#: Скільки архівів кадрів качати одночасно: на далекому боксі один потік
#: бере 1–1.8 МБ/с (Корея, 23.09.2026) — довше, ніж справа читається.
PIPELINE_DL_WORKERS = 3
QUEUE_DIR = Path("/tmp/htrcase/queue")


def _pipeline_supported(code):
    """Раннер в ассетах уміє чергу — інакше справи їдуть звичайним шляхом."""
    return _runner_supports(Path(code) / "htr_case_run.py", "--queue")


def _stage_assets(params):
    """Ассети, скрипти й моделі — ОДИН раз на чергу. (code, модель, голоси, sha)."""
    _ensure_deps()
    url = str(params.get("assets_url") or "")
    if url:
        ...
        if rc != 0:
            raise RuntimeError("curl %s провалився (rc=%s)" % (name, rc))
        size = local.stat().st_size
        print("[htr-case] завантажено %s: %.0f МБ за %.0f с"
              % (name, size / 1e6, time.time() - t), flush=True)
        with tarfile.open(local) as tf:
            tf.extractall(KAGGLE_INPUT)
        local.unlink(missing_ok=True)
    models_root = _slug_dir(str(params.get("models_slug") or "models"))
    scripts_root = _slug_dir(str(params.get("scripts_slug") or "scripts"))
    code = Path("/tmp/htrcase/scripts")
    code.mkdir(parents=True, exist_ok=True)
    for src in scripts_root.rglob("*.py"):
        shutil.copy2(src, code / src.name)
    sha = _verify_scripts(code, params.get("scripts_sha256") or {})
    missing = [n for n in NEEDED_SCRIPTS if not (code / n).is_file()]
    if missing:
        raise RuntimeError("у ассетах бракує скриптів: %s" % missing)
    model_path = _find_model(models_root, str(params.get("model") or "pysar_cyr_v17.pt"))
    ...
    return code, model_path, voice_paths, sha


def _prepare_case(i, case, params, local):
    """Справа №i готова для шардів: кадри розпаковано, стан відновлено."""
    name = str(case.get("case") or "case%d" % i)
    work = KAGGLE_WORKING / _case_slug(name)
    out_dir = work / "out"
    out_dir.mkdir(parents=True, exist_ok=True)
    (work / "logs").mkdir(parents=True, exist_ok=True)
    pages_root = Path("/tmp/htrcase/pq_%03d" % i)
    shutil.rmtree(pages_root, ignore_errors=True)
    pages_root.mkdir(parents=True, exist_ok=True)
    with tarfile.open(local) as tf:
        tf.extractall(pages_root)
    Path(local).unlink(missing_ok=True)
    for tb in [p for p in sorted(pages_root.rglob("*"))
               if p.suffix.lower() in (".tgz", ".tar") or p.name.lower().endswith(".tar.gz")]:
        with tarfile.open(tb) as tf:
            tf.extractall(pages_root)
        tb.unlink(missing_ok=True)
    expected = [p.stem for p in _script_pages(pages_root)]
    limit = int(params.get("limit") or 0)
    if limit:
        expected = expected[:limit]
    if not expected:
        raise RuntimeError("під %s немає кадрів" % pages_root)
    _, last_present = _resume_from_urls(work, list(case.get("resume_urls") or []))
    if params.get("seg_cache"):
        _adopt_legacy_seg_cache(work)
    return {
        "i": i, "case": name, "work": work, "out_dir": out_dir,
        "pages_root": pages_root, "expected": expected,
        "resumed": len(list(out_dir.glob("*.txt"))),
        "sink": _CkptSink(work, case.get("ckpt_urls") or [], last_present, label=name),
        "service_put_url": case.get("service_put_url"),
        "case_key": str(case.get("case_key") or ""),
        "t_ready": time.time(),
    }


def _queue_append(qfile, entry):
    """Дописати рядок у файл черги ОДНИМ записом (шарди читають його на ходу)."""
...
def _case_left_by_all(fleet, i):
    """Кожен живий шард, що вже йде чергою, вийшов зі справи №i (мета дописана).

    Шард, який ще вантажить модель (жодної події черги), справ не тримає: він
    пройде чергу з початку, а закриті справи пропустить. Доти піднятий на
    місце мертвого шард тримав відкритою ВСЮ чергу, поки не завантажиться, —
    18 хв простою флоту на приймачі 23.09.2026, коли 12 нових шардів умирали
    на старті й піднімались знову.

    🔴 Але справу мусить спершу хтось ПРОЙТИ: на старті заходу жоден шард ще не
    в черзі, і без цієї умови закрились би всі підготовлені справи до першої
    сторінки.
    """
    if i not in (fleet.get("qi_left") or {}):
        return False
    for st in fleet["shards"].values():
        if st.get("rc") is not None or not st.get("queue_seen"):
            continue
        if int(st.get("qi_done") or 0) < i:
            return False
    return True


def _live_claims(out_dir):
    """Сторінки справи без тексту, які зараз тримають ЖИВІ процеси.

    Клейм лишається й після прочитання (як у Нишпорці: `claimed_by_others`),
    тож рахується лише той, чия сторінка ще без `.txt`.
    """
    d = Path(out_dir) / "_claims"
    if not d.is_dir():
        return 0
    n = 0
    for c in d.glob("*.claim"):
        if (Path(out_dir) / (c.name[:-len(".claim")] + ".txt")).exists():
            continue
        try:
...
def _pending_pages(ctx, fleet):
    """Непрочитані сторінки відкритої справи (для запасу підготовки)."""
    done = int((fleet.get("qi_pages") or {}).get(ctx["i"], 0))
    return max(0, len(ctx["expected"]) - int(ctx.get("resumed") or 0) - done)


def _finalize_case(ctx, fleet, params, scripts_sha):
    """Закрити справу: фінальний чекпоінт, підсумок, службовий архів, прибирання."""
    work, out_dir = ctx["work"], ctx["out_dir"]
    ctx["sink"].push()
    missing = _missing_pages(out_dir, ctx["expected"])
    meta_pages = {}
    for part in [out_dir / "_htr_meta.json", *sorted(out_dir.glob("_htr_meta.part*.json"))]:
        try:
            meta_pages.update(json.loads(part.read_text(encoding="utf-8")).get("pages", {}))
        except Exception:
            pass
    lines = sorted(v.get("lines") for v in meta_pages.values()
                   if isinstance(v, dict) and isinstance(v.get("lines"), int))
    done_now = max(0, len(list(out_dir.glob("*.txt"))) - int(ctx.get("resumed") or 0))
    summary = {
        "case": ctx["case"],
        "pipeline": True,
        "n_pages_input": len(ctx["expected"]),
        "n_pages_expected": len(ctx["expected"]),
        "n_pages_total": len(meta_pages),
        "n_pages_txt": len(list(out_dir.glob("*.txt"))),
        "n_pages_this_run": done_now,
        "resumed_pages": int(ctx.get("resumed") or 0),
        "missing_pages": missing,
        "quarantined_pages": sorted(_quarantined_stems(out_dir)),
        "complete": not missing,
        "failed_shards": [],
        "model": Path(str(params.get("_model_path") or params.get("model") or "")).name,
        "voices": [Path(v).name for v in params.get("_voice_paths") or []],
        "lines_per_page_median": lines[len(lines) // 2] if lines else None,
        "oom_events": fleet.get("oom_events", 0),
        "oom_events_total": fleet.get("oom_events_total", 0),
        "scripts_sha256": scripts_sha,
        "wall_sec": round(time.time() - ctx["t_ready"], 1),
        "gpu": _gpu_name(),
        "finished": _utc_iso(),
    }
    ...
    ctx["sink"].push()
    _service_to_url(ctx.get("service_put_url"), work)
    shutil.rmtree(str(ctx["pages_root"]), ignore_errors=True)
    print("[htr-case] 🚂 справа %d %s закрита: %d/%d текстів%s"
          % (ctx["i"], ctx["case"], summary["n_pages_txt"], len(ctx["expected"]),
             "" if summary["complete"] else " — неповна, піде в повтор"), flush=True)
    return summary


def _run_pipeline(params, cases, results, retry_pending):
    """Прогнати `cases` конвеєром. Повертає множину номерів (1-based), закритих тут.

    Номер справи — її місце в черзі (`cases`), і він же `index` у результатах
    для наглядача.
    """
    from concurrent.futures import ThreadPoolExecutor

    code, model_path, voice_paths, scripts_sha = _stage_assets(params)
    if not _pipeline_supported(code):
        print("[htr-case] 🚂 раннер в ассетах не знає --queue — справи підуть по одній",
              flush=True)
        return set()
    params = dict(params)
    params["_model_path"] = str(model_path)
    params["_voice_paths"] = voice_paths
    shutil.rmtree(QUEUE_DIR, ignore_errors=True)
    QUEUE_DIR.mkdir(parents=True, exist_ok=True)
    (QUEUE_DIR / "logs").mkdir(parents=True, exist_ok=True)
    qfile = QUEUE_DIR / "queue.jsonl"
    qfile.write_text("", encoding="utf-8")

    seg_cache = bool(params.get("seg_cache"))
    base = [
    ...
    if int(params.get("limit") or 0):
        base += ["--limit", str(int(params["limit"]))]
    if int(params.get("batch") or 0):
        base += ["--batch", str(int(params["batch"]))]
    if int(params.get("voice_batch") or 0) > 1:
        base += ["--voice-batch", str(int(params["voice_batch"]))]
    if not seg_cache:
        base += ["--no-seg-cache"]
    ...
    if params.get("no_gpu_sato"):
        base += ["--no-gpu-sato"]
    if params.get("gpu_lock"):
        base += ["--gpu-lock", "/tmp/htrcase/gpu.lock"]
    ...
    shards_max = max(1, min(MAX_SHARDS, int(params.get("shards_max") or params.get("shards")
                                            or _auto_shards(1.5))))
    ...
    threads = int(params.get("threads_per_shard") or 0) or max(
        1, min(8, _usable_cores() // max(shards, shards_max)))
    env = dict(os.environ)
    env["PYTHONUNBUFFERED"] = "1"
    for var in ("OMP_NUM_THREADS", "MKL_NUM_THREADS", "OPENBLAS_NUM_THREADS",
                "NUMEXPR_NUM_THREADS", "VECLIB_MAXIMUM_THREADS"):
        env[var] = str(threads)
    print("[htr-case] 🚂 конвеєр: %d справ, %d шардів (стеля %d), %d потоків на шард, "
          "качаю по %d наперед" % (len(cases), shards, shards_max, threads,
                                   PIPELINE_DL_WORKERS), flush=True)

    t0 = time.time()
    total = sum(int(c.get("estimated_n_pages") or 0) for c in cases)
    fleet = {
        "schema": 1,
        # Не ім'я справи: прогрес конвеєра — про всю чергу, і наглядач не має
        # класти його в стан однієї справи.
        "case": "черга: %d справ" % len(cases),
        "case_index": 1,
        "cases_total": len(cases),
        "phase": "running",
        "n_pages_expected": total,
        "started": _utc_iso(),
        "shards": {},
        "oom_events": 0,
        "n_gpus": n_gpus,
    ...
    # 🔴🔴 Чекпоінти ПІД ЧАС справи, а не лише при закритті. Доти конвеєр писав
    # один знімок на закритті: велика справа годину жила лише на диску бокса.
    checkpoint_sec = max(30, int(params.get("checkpoint_sec") or 120))
    fleet["ckpt"] = {"ok": 0, "files": 0, "last_ok": None, "exhausted": False,
                     "tail_rewrites": 0}

    def push_all():
        with lock:
            live = [c for i, c in prepared.items() if i not in closed]
            every = [c["sink"] for c in prepared.values()]
        for c in live:
            try:
                c["sink"].push()
            except Exception as exc:
                print("[htr-case] ⚠ чекпоінт %s упав: %r" % (c["case"], exc), flush=True)
        _ckpt_health(fleet["ckpt"], every)

    def checkpointer():
        _checkpoint_loop(stop, checkpoint_sec, push_all)

    def download(i):
        url = str(cases[i - 1].get("pages_url") or "")
        local = Path("/tmp/htrcase/pdl_%03d" % i) / _url_name(url)
        local.parent.mkdir(parents=True, exist_ok=True)
        rc = _download_with_heartbeat(url, local, local.name, heartbeat=False)
        if rc != 0:
            # Дрібні справи конвеєра повторно не качаються з цієї теки — огризок
            # лише займав би диск під томи, що читаються.
            _drop_download(local)
            raise RuntimeError("curl %s провалився (rc=%s)" % (local.name, rc))
        return local

    def preparer():
        pool = ThreadPoolExecutor(max_workers=PIPELINE_DL_WORKERS)
        futures = {}
        try:
            for i in range(1, len(cases) + 1):
                # Вікно — запас непрочитаних сторінок, а не число незакритих
                # справ: повільна справа в голові черги більше не зупиняє решту.
                while not stop.is_set():
                    with lock:
                        open_ctx = [c for j, c in prepared.items() if j not in closed]
                        pending = sum(_pending_pages(c, fleet) for c in open_ctx)
                    live = sum(1 for st in list(fleet["shards"].values())
                               if st.get("rc") is None)
                    want = max(PIPELINE_BUFFER_MIN, PIPELINE_BUFFER_PER_SHARD * live)
                    if len(open_ctx) < PIPELINE_OPEN_MAX and pending < want:
                        break
                    time.sleep(0.5)
                if stop.is_set():
                    return
                for j in range(i, min(len(cases), i + 2 * PIPELINE_DL_WORKERS) + 1):
                    if j not in futures:
                        futures[j] = pool.submit(download, j)
                try:
                    local = futures.pop(i).result()
                    ctx = _prepare_case(i, cases[i - 1], params, local)
                except Exception as exc:
                    print("[htr-case] ✗ справа %d %s не готова: %r — піде в повтор"
                          % (i, cases[i - 1].get("case"), exc), flush=True)
                    with lock:
                        closed.add(i)
                    _record_result(results, i, {"case": cases[i - 1].get("case"),
                                                "complete": False, "error": repr(exc)},
                                   1, retry_pending)
                    continue
                with lock:
                    prepared[i] = ctx
                    fleet["resumed_pages"] += ctx["resumed"]
                    _BOX_RESUMED[0] += ctx["resumed"]
                seg_dir = str(ctx["work"] / SEG_CACHE_ARC) if seg_cache else ""
                _queue_append(qfile, {"case_dir": str(ctx["pages_root"]),
                                      "out_dir": str(ctx["out_dir"]),
                                      "case_key": ctx["case_key"],
                                      "seg_cache_dir": seg_dir, "index": i})
        finally:
            _queue_append(qfile, {"end": True})
            pool.shutdown(wait=False, cancel_futures=True)

    def finalizer():
        # 🔴 Закривається БУДЬ-ЯКА справа, з якої всі вийшли, а не лише голова
        # черги: доти одна повільна сторінка тримала закритими всі наступні.
        while len(closed) < len(cases):
            with lock:
                open_ids = sorted(j for j in prepared if j not in closed)
            if not open_ids:
                if stop.is_set():
                    return
                time.sleep(1.0)
                continue
            shards_now = list(fleet["shards"].values())
            fleet_gone = bool(shards_now) and all(st.get("rc") is not None
                                                  for st in shards_now)
            progressed = False
            for j in open_ids:
                ctx = prepared[j]
                # Неповна справа теж закривається, щойно всі живі шарди з неї
                # вийшли: цим проходом її сторінок уже ніхто не прочитає
                # (клейм померлого, отруйна сторінка); доганяє повтор томів.
                # Живий клейм — сторінка ще в руках (зокрема шарда, що лише
                # увійшов і не встиг подати подію).
                if not fleet_gone and (not _case_left_by_all(fleet, j)
                                       or _live_claims(ctx["out_dir"])):
                    continue
                try:
                    summary = _finalize_case(ctx, fleet, params, scripts_sha)
                except Exception as exc:
                    summary = {"case": ctx["case"], "complete": False, "error": repr(exc)}
                _record_result(results, j, summary, 1, retry_pending)
                with lock:
                    # `case_index` лишається 1: `pages_done` конвеєра — уже вся
                    # черга (див. наглядач).
                    closed.add(j)
                progressed = True
            if not progressed:
    ...
    prep_t = threading.Thread(target=preparer, name="pipeline-prep", daemon=True)
    fin_t = threading.Thread(target=finalizer, name="pipeline-fin", daemon=True)
    ck_t = threading.Thread(target=checkpointer, name="pipeline-ckpt", daemon=True)
    prep_t.start()
    fin_t.start()
    ck_t.start()

    def out_dir_of(state):
        ctx = prepared.get(int(state.get("qi") or 0))
        return ctx["out_dir"] if ctx else QUEUE_DIR

    for k in range(shards):
        _start_shard(k, REG_DENOM, base, env, QUEUE_DIR / "logs", fleet)
    regulate = bool(params.get("regulate", True)) and _runner_supports(
        code / "htr_case_run.py", "_drain")
    regulator = (_FleetRegulator(fleet, out_dir=QUEUE_DIR, base=base, env=env,
                                 logs_dir=QUEUE_DIR / "logs", denom=REG_DENOM,
                                 ceiling=shards_max, cores=_usable_cores(), t0=t0,
                                 memory=None, cores_per_shard=reg_cores)
    ...
    try:
        _watch_fleet(fleet, QUEUE_DIR, shards=shards, stall_sec=stall_sec,
                     restart_max=restart_max, base=base, env=env,
                     logs_dir=QUEUE_DIR / "logs", t0=t0, regulator=regulator,
                     out_dir_of=out_dir_of)
    finally:
        stop.set()
        prep_t.join(timeout=120)
        fin_t.join(timeout=1800)
    # Флот вийшов: справи, яких фіналізатор не встиг закрити (підготовка
    # впала посередині, шарди злиті), закриваються тут — як є, а неповні
    # піде доганяти звичайний повтор.
    for i in sorted(prepared):
        if i not in closed:
            try:
                summary = _finalize_case(prepared[i], fleet, params, scripts_sha)
            except Exception as exc:
                summary = {"case": prepared[i]["case"], "complete": False, "error": repr(exc)}
            _record_result(results, i, summary, 1, retry_pending)
            closed.add(i)
    for i in range(1, len(cases) + 1):
        if i not in closed:
            _record_result(results, i, {"case": cases[i - 1].get("case"), "complete": False,
                                        "error": "конвеєр не дійшов до справи"},
                           1, retry_pending)
            closed.add(i)
    _ckpt_health(fleet["ckpt"], [c["sink"] for c in prepared.values()])
    done = sum(1 for n in closed if n <= len(results) and results[n - 1].get("complete"))
    print("[htr-case] 🚂 конвеєр завершено за %.0f с: %d/%d справ повні"
          % (time.time() - t0, done, len(cases)), flush=True)
    return closed
...
def _main_inner(params):
    ...
    results = []
    # 🚂 Дрібні справи — першими й конвеєром (див. `_run_pipeline`); великі
    # лишаються за ними звичайним шляхом. Порядок у черзі — і є `index`
    # результатів для наглядача, тож переставляємо ДО всього іншого.
    small = [c for c in cases
             if 0 < int(c.get("estimated_n_pages") or 0) <= PIPELINE_MAX_PAGES]
    use_pipeline = bool(params.get("pipeline", True)) and len(small) >= 2
    if use_pipeline:
        cases = small + [c for c in cases if c not in small]
    ...
    def run_one(i, attempt):
        ...
        merged["_skip_assets"] = i > 1 or bool(params.get("_assets_staged"))
        ...
        try:
            ...
            # Службові файли їдуть у сховище ТУТ, а не в кінці заходу: якщо
            # наступна справа завалить бокс, діагностика цієї вже вдома.
            # 🔴 Тека БОКСУ (`_out_root`), а не `out_dir`: останнє — адреса
            # вдома, і логів шардів там немає за побудовою.
            _service_to_url(merged.get("service_put_url"),
                            merged.get("_out_root"))
            _drop_case_frames(merged)

    i = 0
    retries_left = retry_passes
    if use_pipeline:
        closed = _run_pipeline(params, queue[:len(small)], results, retry_passes > 0)
        if closed:
            # ассети вже на боксі — звичайний шлях їх не качає
            params = dict(params)
            params["_assets_staged"] = True
            i = len(small)
    while True:
        while i < len(queue):
            i += 1
...
def _run_case(params):
    ...
    for url, dest in ((assets_url, KAGGLE_INPUT),
        ...
        ready = prefetched.take(url, resume_to=local) if prefetched is not None else None
        if ready is not None:
            local, rc = ready, 0
        else:
            rc = _download_with_heartbeat(url, local, name)
        if rc != 0:
            # 🔴 У текст іде ІМ'Я файла, а не підписане посилання. Повне
            # посилання R2 — 368 символів, і разом із обгорткою це 392 при
            # ліміті запису 400: причина трималась на восьми символах, а в
            # наглядача (300) не влазила вже точно. 21.09.2026 три справи
            # поспіль упали, і жоден запис не зберіг ні rc, ні тексту —
            # діагноз довелось вгадувати, і вгадали неправильно.
            raise RuntimeError("curl %s провалився (rc=%s)" % (name, rc))
        size = local.stat().st_size
        dt = max(0.001, time.time() - t)
        if ready is not None:
    ...
    got, last_present = _resume_from_urls(work, resume_urls)
    resumed += got
    if seg_cache:
    ...
    ck_seen = {}
    ck_sink = _CkptSink(work, ckpt_urls, ck_state["rounds"]) if ckpt_urls else None
    ...
    def checkpoint(tag=""):
        ...
        t = time.time()
        if ck_sink is not None:
            # Ковзний останній слот: вичерпання немає (див. `_CkptSink`).
            n = ck_sink.push()
            nxt = ck_sink.rounds
        else:
        ...
        ck_state["exhausted"] = bool(ck_sink is not None and ck_sink.stuck)
        ck_state["files"] += n
        ...
        ck_health["tail_rewrites"] = ck_sink.tail_rewrites if ck_sink is not None else 0
        ck_state["last_sec"] = round(time.time() - t, 1)
        if n and ck_sink is None:
            print("[htr-case] чекпоінт%s → ckpt_%04d.tgz: %d файл(ів) за %.1f с"
                  % (tag, ck_state["rounds"], n, ck_state["last_sec"]), flush=True)

    def checkpointer():
        _checkpoint_loop(stop, checkpoint_sec, checkpoint)
