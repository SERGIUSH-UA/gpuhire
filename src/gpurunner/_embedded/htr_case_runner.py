...
def _download_ranged(url, local, name, total, parts, floor, heartbeat, min_free_bytes,
    ...
    for i, path in enumerate(paths):
        ...
        procs.append(subprocess.Popen(
            ["curl", "-fsSL", "--retry", "2", "--max-time", "7200",
             "--speed-limit", str(max(16384, floor // parts)), "--speed-time", "60",
             "-r", "%d-%d" % (start, end), "-o", str(path), url]))
...
def _download_with_heartbeat(url, local, name, tries=DOWNLOAD_TRIES, heartbeat=True,
    ...
    Обрив НЕ скидає привезене: повтор продовжує з наявного байта діапазонними
    з'єднаннями, і лише якщо продовження теж не пішло — качання починається з
    нуля з м'якшою підлогою швидкості (`DOWNLOAD_RETRY_MIN_BPS`).
    ...
    for attempt in range(1, tries + 1):
        ...
        if rc == _ESCALATE:
            ...
            if rc not in (0, -2):
                # 🔴 Удруге паралель не пробуємо: якщо вона теж не пішла, річ у
                # маршруті, а не в числі з'єднань, і повтор лише подвоїть втрату.
        ...
        print("[htr-case] ⚠ качання %s: curl rc=%s (спроба %d/%d, привезено %.0f МБ)"
              % (name, rc, attempt, tries, got / 1e6), flush=True)
        ...
        #
        # Умова безпеки одна: файл на диску мусить бути ПРЕФІКСОМ об'єкта.
        # Так і є — і один потік (`curl -o`, послідовний запис), і склейка
        # діапазонів (частини дописуються строго по черзі) лишають саме префікс.
        if total and 0 < got < total and attempt < tries:
            left = total - got
            n = max(2, min(DOWNLOAD_PARTS, left // DOWNLOAD_MIN_PART_BYTES))
            print("[htr-case] ⏬ %s: продовжую з %.0f МБ, лишилось %.0f МБ у %d з'єднань"
                  % (name, got / 1e6, left / 1e6, n), flush=True)
            rc = _download_ranged(url, local, name, total, int(n),
                                  _download_floor(attempt + 1), heartbeat,
                                  min_free_bytes, attempt + 1, begin=got)
            if rc == -2:
                return -2
            if rc == 0:
                ...
                return 0
            print("[htr-case] ⚠ продовження %s не вдалось (rc=%s) — качаю з нуля"
                  % (name, rc), flush=True)
        try:
            local.unlink()
        except OSError:
