...
from gpurunner.core.offer_score import Need, Selection, select_offers, target_cores_floor
...
_SSH_AUTH_GRACE = 240
#: Спроби проби заліза і пауза між ними, с (див. `probe_box`).
_PROBE_ATTEMPTS = 3
_PROBE_RETRY_SEC = 20
...
#: Порти, які просимо прокинути з контейнера НАЗОВНІ при кожній оренді.
#: 🔴 Тільки при оренді: доданий пізніше порт вимагає перестворення інстансу,
#: тобто іншої машини за інші гроші. Зараз тут один — склад `htr/origin.py`,
#: яким їдуть і дані на машину, і результат додому.
DIRECT_PORTS = (8752,)

_NUMERIC_PROBE_KEYS = frozenset(
    {
        "cores", "cores_all", "ram_gb", "vram_total_mb", "vram_free_mb",
        "vram_free_min_mb", "n_gpus", "disk_free_gb", "net_bps", "net_par_bps",
...
class VastBackend(Backend):
        ...
        **після SSH, але до заливки даних і pip**. Саме туди наглядач вішає
        ...
        try:
            ...
            try:
                ...
                self._upload_inputs(client, inputs, handle)
    ...
    def rent_box(
        ...
        *,
        ...
        payload = _instance_payload(
            ...
            num_gpus=cards, open_ports=DIRECT_PORTS,
        )
        print(
    ...
    def fetch_outputs(self, handle: JobHandle, out_dir: Path,
                      *, deadline_sec: float = FETCH_DEADLINE_SEC,
                      only: Sequence[str] = ()) -> list[Path]:
        """Забрати вивід ЦІЛКОМ, обходом теки. З ТАЙМАУТАМИ.

        🔴 Це шлях аварії, а не робочий. Штатно тіло результату приїжджає зі
        сховища одним об'єктом на чекпоінт, а службові файли — одним архівом на
        справу; сюди наглядач заходить лише коли у сховищі не виявилось нічого.
        Причина в обертах: одна справа це 2379 дрібних файлів, і на черзі з
        восьми пофайловий SFTP дає 11-22 хвилини, скільки б не було мегабайтів.
        Тут же жив `only_meta=True` — «дотяг лише службового», який на черзі
        вироджувався в ~2600 обертів і вішав забір понад 30 хвилин при цілому
        результаті вдома (23.09.2026). Його немає: службові файли везе сховище.
        ...
        try:
            ...
            try:
                ...
                if only:
                    # Добір лише названих справ (`<working>/<справа>`): решта вже
                    # удома зі сховища, і обхід усієї теки повторив би її даремно.
                    written = []
                    for slug in only:
                        written += self._download_tree(
                            sftp, f"{REMOTE_WORKING}/{slug}", out_dir / slug,
                ...
                # Логи шардів тут приїжджають самі: вони лежать усередині
                # `<working>/<справа>/logs`, тобто в тому ж обході теки.
                return written
            finally:
    ...
    def search_offers(
        ...
        *,
        ...
        if price_is_total and max_price:
            # Режим цілі: `max_price` — стеля ВСЬОГО бокса (похідна від цілі й
            # стелі тисячі сторінок). Стеля НА КАРТУ лишається незнімною й
            # перевіряється нижче поділом на число карт — у запиті Vast такого
            # поділу нема, а без нього багатокарткові бокси, що дають 12+
            # шардів, не потрапляли у видачу взагалі.
        ...
        offers = [o for o in offers
                  if float(o.get("dph_total") or 0)
                  <= ABSOLUTE_MAX_DPH * max(1, int(o.get("num_gpus") or 1)) + 1e-9]
        if exclude:
    ...
    def find_candidates(
        ...
        *,
        ...
        """Кандидати на оренду: пам'ять реєстру + ціль темпу + ранжування за ціною.
        ...
        1. **Адресно по зірках** — `machine_id.eq` окремим запитом на кожну
           машину, яка вже відпрацювала: «був добрий бокс — знайди саме його».
        2. **Ринок без забанених** — двома порядками видачі (ядра спаданням і
           ціна зростанням) з підлогою ядер, похідною від цілі.
        3. **Вибір** — `select_offers`: ціль, бюджет і строк не
           послаблюються ніколи.
        ...
        for star in boxes.starred(gpu=None, limit=8, now=now):
            try:
                found = self.search_offers(
                    gpu=gpu, max_price=(need.max_dph if need.target_pph > 0 else max_price),
                    disk_gb=need.disk_gb, num_gpus=num_gpus,
                    limit=4, machine_ids=[star.machine_id],
                    price_is_total=need.target_pph > 0,
                )
            except BackendError as e:
        ...
        # 🔴🔴 Підлоги запиту — ВІД ЦІЛІ, а не від тірів. Машина, якій на ціль
        # бракує ядер, не пройде вибору за жодних умов, тож і тягнути її з ринку
        # нема чого: вона лише витісняє з ліміту видачі ті, що проходять.
        # Без цілі (старий режим) — колишні 8 ядер і місце під один шард.
        floor_cores, floor_vram, pass_limit = 8.0, need.gb_per_shard, limit
        if need.target_pph > 0:
            floor_cores = max(8.0, target_cores_floor(need))
            pass_limit = max(limit, 100)
        # Стеля ціни години з ціллю — похідна (`Need.max_dph`), а не $0.365:
        # машина, що ДАЄ ціль, може коштувати стільки, скільки дозволяє стеля
        # тисячі сторінок, — інакше багатокарткові бокси на 12+ шардів не
        # потрапляли у видачу взагалі.
        price_cap = max_price
        if need.target_pph > 0 and need.max_dph > 0:
            price_cap = need.max_dph
        # Два порядки видачі: за ЯДРАМИ спаданням (швидкі машини) і за ЦІНОЮ
        # зростанням (дешеві серед придатних). Видача Vast обрізана лімітом,
        # тож один порядок завжди бачить лише свій край ринку.
        for take, order in (
            (pass_limit, [["cpu_cores_effective", "desc"]]),
            (pass_limit, None),
        ):
            try:
                found = self.search_offers(
                    gpu=gpu, max_price=price_cap, disk_gb=need.disk_gb, num_gpus=num_gpus,
                    limit=take, min_cpu=floor_cores, min_vram_gb=floor_vram,
    ...
    def _upload_text(self, client: Any, remote_path: str, text: str) -> None:
        ...
        🔴🔴 `set_pipelined(True)` — не оптимізація, а виправлення 25 хвилин
        оренди. Без нього paramiko пише СИНХРОННО шматками по 32 КБ: кожен
        шматок чекає підтвердження, тож ціна файла — не смуга, а
        `розмір / 32 КБ × круговий оберт`. Заміряно 23.09.2026: `job.py` на
        48 МБ (черга з 194 справ вшивала 121 832 presigned-посилання) поїхав до
        Кореї півтори тисячі обертами й забрав 25 хвилин, а в лозі це виглядало
        як тиша. Розмір посилань полагоджено окремо (`CKPT_PAGES_PER_SLOT`), але
        синхронний запис — це вада сама собою: наступний великий файл заплатив
        би те саме.
        """
    ...
    def _upload_inputs(self, client: Any, inputs: dict[str, Path],
                       handle: JobHandle | None = None) -> None:
        """Залити локальні теки входів — ОДНИМ архівом на теку, системним `scp`.

        🔴🔴 Тут стояв paramiko SFTP, і це найгірший канал з усіх наявних:
        заміряно на орендованому боксі 20.09.2026 на 150 МБ — SFTP paramiko
        **0.48 МБ/с** проти 7.56 у системного `scp` і 20.07 у HTTP. Причина
        відома: paramiko жене дані одним вікном. Плюс на кожен файл ішло ДВА
        обміни (`put` і `stat`), тобто тека з тисячі кадрів платила дві тисячі
        кругових обертів через океан.
        ⚠ Я двічі звітував, що «paramiko прибрано з передачі даних», маючи на
        увазі забір і службові файли, — а цей шлях лишався. Тому тут явно:
        paramiko більше немає, канал — `scp`.

        Чому `scp`, а не HTTP, яким їде решта: склад (`htr/origin.py`) на цей
        момент ЩЕ НЕ ПІДНЯТИЙ — його ставлять ворота, а вони спрацьовують
        пізніше; а цей шлях узагалі працює лише там, де ні бакета, ні складу
        немає (при них теки входів порожні за побудовою). Тобто вибір тут між
        0.48 і 7.56, і він однозначний.

        Приймач подвійний: розмір архіву НА МАШИНІ (його звіряє `push`) і
        КІЛЬКІСТЬ розпакованих файлів. Обірвана заливка інакше лишає теку, що
        виглядає повною, і виявляється це вже на оплачуваній карті.
        """
        if not inputs:
            return
        import tarfile
        ...
        if handle is None:                     # pragma: no cover — захисний шлях
            raise BackendError("заливка входів без handle: нема куди стукати scp")
        ep = bt.endpoint_of(self, handle)
        for slug, local in inputs.items():
            ...
            with tempfile.TemporaryDirectory(prefix="grinput-") as tmp:
                ball = Path(tmp) / f"{slug}.tar"
                with tarfile.open(ball, "w") as tf:
                    for p in files:
                        tf.add(p, arcname=p.name)
                remote_tar = f"/tmp/_in_{slug}.tar"
                self._exec(client, f"mkdir -p {remote_dir}")
                bt.push(ep, ball, remote_tar,
                        verify_with=lambda cmd: self._exec(client, cmd))
            landed = self._exec(
                client,
                f"tar -xf {remote_tar} -C {remote_dir} && rm -f {remote_tar} "
                f"&& ls -1 {remote_dir} | wc -l")
            got = int((landed or "0").strip().splitlines()[-1] or 0)
            if got < len(files):
                raise BackendError(
                    f"{slug}: розпакувалось {got} файл(ів) із {len(files)} — "
                    f"заливка обірвалась. На машині це виглядало б як неповна "
                    f"тека входу вже під час роботи.")
    ...
    def instance_charges(self, start_ts: float, end_ts: float) -> list[dict[str, Any]]:
        """Списання Vast по інстансах за період — ті самі числа, що на порталі.

        Кожен запис: `instance`, `label`, `amount` і розклад `items`
        (gpu, disk, bwd — трафік завантаження, bwu — вивантаження). Трафіку
        немає в ціні за годину, тож лише цей рахунок каже, скільки заплачено.
        """
        import urllib.parse

        out: list[dict[str, Any]] = []
        params: dict[str, Any] = {
            "select_filters": {"day": {"gte": int(start_ts), "lte": int(end_ts)},
                               "type": {"in": ["instance"]}},
            "latest_first": True, "limit": 100, "format": "tree"}
        for _ in range(50):
            query = "&".join(f"{k}={urllib.parse.quote_plus(json.dumps(v))}"
                             for k, v in params.items())
            data = self._request("GET", f"/charges/?{query}")
            for row in data.get("results") or []:
                src = str(row.get("source") or "")
                out.append({
                    "instance": src.removeprefix("instance-"),
                    "label": str((row.get("metadata") or {}).get("label") or ""),
                    "amount": float(row.get("amount") or 0),
                    "day": row.get("start"),
                    "items": {str(i.get("type")): float(i.get("amount") or 0)
    ...
    def request_checkpoint(self, handle: JobHandle, *, wait_sec: int = 120) -> str:
        """Попросити раннер залити чекпоінт ЗАРАЗ і дочекатись відповіді.

        Один SSH-виклик; текст іде в сховище HTTP-заливкою раннера. Повертає
        `done`, `no_runner` (циклу чекпоінтів немає — раннер завершився, і
        фінальні чекпоінти вже в сховищі) або `timeout`.
        """
        n = max(1, int(wait_sec // 2))
        cmd = (
            "P=$(cat /tmp/htrcase/ckpt.pid 2>/dev/null); "
            "{ [ -n \"$P\" ] && kill -0 \"$P\" 2>/dev/null; } || exit 3; "
            "rm -f /tmp/htrcase/ckpt_done; touch /tmp/htrcase/ckpt_now; "
            f"i=0; while [ $i -lt {n} ]; do "
            "[ -f /tmp/htrcase/ckpt_done ] && exit 0; sleep 2; i=$((i+1)); done; exit 4"
        )
        client = self._ssh(handle)
        try:
            _, out, _ = client.exec_command(cmd, timeout=wait_sec + 30)
            rc = out.channel.recv_exit_status()
        finally:
            client.close()
        return {0: "done", 3: "no_runner"}.get(rc, "timeout")

    def case_files(self, handle: JobHandle, slug: str) -> dict[str, int]:
        """Файли теки справи на машині: відносний шлях → розмір. Один обмін.

        Клейми не входять: це стан флоту цієї машини, удома вони нічого не значать.
        """
        client = self._ssh(handle)
        try:
            _, out, _ = client.exec_command(
                f"cd {REMOTE_WORKING}/{slug} 2>/dev/null && "
                f"find . -type f -not -path '*/_claims/*' -printf '%P\\t%s\\n'",
                timeout=120)
            files: dict[str, int] = {}
    ...
    def fetch_case_files(self, handle: JobHandle, slug: str, rels: Sequence[str],
                         dest: Path) -> int:
        """Забрати названі файли справи ОДНИМ архівом у `dest`. Повертає їх число.

        Пофайловий SFTP через океан платить оберт на кожен файл, а справа — це
        тисячі дрібних файлів; архів — один обмін.
        """
        import io
        import tarfile
        ...
        if not rels:
            return 0
        client = self._ssh(handle)
        lst, arc = f"/tmp/_delta_{slug}.lst", f"/tmp/_delta_{slug}.tgz"
        try:
            sftp = client.open_sftp()
            try:
                with contextlib.suppress(Exception):
                ...
                _, out, _ = client.exec_command(
                    f"cd {REMOTE_WORKING}/{slug} && tar czf {arc} -T {lst}; echo $?",
                    timeout=900)
                out.read()
                dest.mkdir(parents=True, exist_ok=True)
                local = dest.parent / f"_delta_{slug}.tgz"
                sftp.get(arc, str(local))
                try:
    ...
    def http_endpoint(self, handle: JobHandle, port: int = 0) -> dict[str, Any] | None:
        """Куди стукати ЗНАДВОРУ в порт, прокинутий із контейнера.

        🔴 Зовнішній порт майже ніколи не дорівнює внутрішньому: Vast мапить
        його на випадковий вільний порт хоста, і знає про це лише сам інстанс.
        Тому адреса не вгадується, а читається — `ports` інстансу плюс
        `public_ipaddr`. `None` означає «порту немає або він ще не мапнутий», і
        це нормальний стан: доставка тоді йде запасним плечем.
        """
        from gpurunner.htr.box_transport import ORIGIN_PORT

        want = int(port or ORIGIN_PORT)
        inst = self._instance(handle)
        if inst is None:
            return None
        ports = inst.get("ports") or {}
        bound = ports.get(f"{want}/tcp") or ports.get(str(want)) or []
        if isinstance(bound, dict):
            bound = [bound]
        outside = 0
        for item in bound:
            if isinstance(item, dict) and str(item.get("HostPort") or "").isdigit():
                outside = int(item["HostPort"])
                break
        host = str(inst.get("public_ipaddr") or "").strip()
        if not outside or not host:
            return None
        return {"host": host, "port": outside, "inside": want}
    ...
    def probe_box(self, handle: JobHandle, *, net_probe_url: str = "") -> dict[str, Any]:
        ...
        # 🔴 Кілька спроб. SSH свіжого боксу вже відповів (`_wait_for_ssh`), але
        # проксі Vast може один раз зірвати наступне з'єднання, поки контейнер
        # доставляє пакети: одна така відмова гасила машину через хвилину з
        # вироком `ssh_unreachable` (52409272, 24.09.2026, ssh3 — 68 с від оренди).
        # Відмова ключа не повторюється: на неї окреме правило.
        last: Exception | None = None
        for attempt in range(_PROBE_ATTEMPTS):
            if attempt:
                time.sleep(_PROBE_RETRY_SEC)
            try:
                client = self._ssh(handle, timeout=60)
            except SshAuthRejected:
                raise
            except BackendError as e:
                last = e
                continue
            try:
                ...
                client.close()
            probe = self.parse_probe(raw)
            if "cores" in probe or "gpu" in probe:
                return probe
            last = BackendError("проба повернула порожній вивід")
        err = last if isinstance(last, BackendError) else BackendError(
            f"проба заліза не вдалась за {_PROBE_ATTEMPTS} спроби: {last!r}")
        if not getattr(err, "outcome", ""):
    ...
    def parse_probe(raw: str) -> dict[str, Any]:
        ...
        probe["net_par_mbps"] = float(probe.get("net_par_bps") or 0) * 8 / 1_000_000
        return probe
...
def _instance_payload(
    ...
    onstart: str, num_gpus: int, open_ports: Sequence[int] = (),
) -> dict[str, Any]:
    """Тіло `PUT /asks/<offer>/` — одне на job і на голу оренду.

    🔴 `open_ports` — порти, які Vast мусить прокинути з контейнера назовні.
    Просити їх треба ПРИ ОРЕНДІ: доданий пізніше порт вимагає перестворення
    інстансу, тобто нової машини за нові гроші. Проситься завжди, навіть коли
    транспорт цього заходу — бакет: зайвий прокинутий порт не коштує нічого
    (склад на ньому просто не піднімають), а його відсутність коштує оренди.
    Форма саме така, як розуміє Vast: ключ — аргумент docker, значення — «1».
    """
    env: dict[str, Any] = {f"-p {int(p)}:{int(p)}": "1" for p in open_ports}
    return {
        "client_id": "me",
        "image": image,
        "disk": float(disk_gb),
...
' "$U" 2>/dev/null
    echo "net_rc=$?"
    # 🔴 ДРУГЕ число: той самий обсяг, але ВІСЬМОМА діапазонами. Транспорт на
    # боксі ескалює до 8 з'єднань нижче 4 МБ/с, тобто однопотокова проба міряє
    # не те, чим потім качають, і систематично занижує саме там, де транспорт
    # виграє. Звідси й бралось «раз 10 МБ/с, раз 1.5»: часто це той самий
    # канал, зміряний одним потоком і використаний вісьмома.
    # Обсяг навмисно ТОЙ САМИЙ (32 МБ): порівнюються два способи, а не два
    # розміри, тож їхнє відношення і є відповідь «чи дає тут паралельність».
    T0=$(date +%s%N 2>/dev/null); case "$T0" in *N*|"") T0=$(($(date +%s)*1000000000));; esac
    i=0
    while [ $i -lt 8 ]; do
      b=$((i*4194304)); e=$((b+4194303))
      curl -o /dev/null -s --max-time 12 -r $b-$e "$U" 2>/dev/null &
      i=$((i+1))
    done
    wait
    T1=$(date +%s%N 2>/dev/null); case "$T1" in *N*|"") T1=$(($(date +%s)*1000000000));; esac
    D=$((T1-T0)); [ "$D" -lt 1000000 ] && D=1000000
    echo "net_par_bps=$((33554432*1000000000/D))"
  else
    echo "net_bps=0"
  fi
else
