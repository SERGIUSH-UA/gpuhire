...
⚠ На PyPI пакет і репозиторій звуться **`gpuhire`**, а команда в терміналі,
модуль і змінні оточення — `gpurunner`. Ім'я `gpurunner` на PyPI зайняте
схожим проєктом (`gpu-runner`), і майданчик не дав його взяти; перейменовувати
команду заради цього не стали. Тобто `pip` і `git clone` кажуть одне ім'я, а
`--help` і `GPURUNNER_*` — інше, і це не помилка.
