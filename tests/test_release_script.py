...
def test_checks_refresh_remote_tracking_first() -> None:
    """🔴 Без `fetch` дві перевірки читають застарілі гілки відстеження.

    `origin/<ref>` збрехав би про «не запушено», а `--force-with-lease` у
    `cut` звіряється з `origin/public` — і зі застарілим відхилив би пуш
    посеред релізу. У репозиторії, де паралельно працює інша сесія, застаріле
    відстеження це типовий стан, а не рідкість.
    """
    import inspect

    calls = _git_calls(inspect.getsource(rel.check))
    assert any(args and args[0] == "fetch" for args in calls), (
        "`check` мусить оновити гілки відстеження перед тим, як їх читати")
