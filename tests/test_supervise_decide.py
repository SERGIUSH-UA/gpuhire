...
@pytest.mark.parametrize("state", ["exited", "stopped", "gone"])
...
def test_one_offline_reading_on_a_working_box_is_not_death() -> None:
    """🔴🔴 23.09.2026: бокс 42769 знищено після ОДНОГО рядка «offline», а його
    раннер ще три хвилини вивантажував прогрес. М'який стан — смерть лише
    тоді, коли тримається І бокс справді мовчить."""
    one = decide(Obs(progress=progress(), instance_state="offline",
                     dead_state_streak=1, ssh_ok=True), CFG)
    assert one[0] != "destroy_dead_box"
    held_but_talking = decide(Obs(progress=progress(), instance_state="offline",
                                  dead_state_streak=5, ssh_ok=True,
                                  progress_age_sec=10.0), CFG)
    assert held_but_talking[0] != "destroy_dead_box"
    dead = decide(Obs(progress=progress(), instance_state="offline",
                      dead_state_streak=3, ssh_ok=False), CFG)
    assert dead[0] == "destroy_dead_box"


# ---- обіцянка проти факту ----------------------------------------------------


def _target_cfg() -> Cfg:
    from dataclasses import replace
    return replace(CFG, target_pph=5000.0)


def test_a_box_that_keeps_missing_the_promise_is_replaced() -> None:
    """🔴🔴 Вибір обіцяв 5000 стор/год; бокс 15 хв дає 2851 — не варіант.
    Доти прогноз воріт не порівнювався з фактом ніколи."""
    obs = Obs(progress=progress(), box_pages=713, box_sec=900.0, underdeliver_streak=3)
    action, why = decide(obs, _target_cfg())
    assert action == "underdelivers"
    assert "2852" in why or "2851" in why


def test_a_short_dip_is_not_underdelivery() -> None:
    """Перехід між справами дає миттєвий провал темпу (356 стор/год о 14:55 при
    2851 до і після): рішення — лише на стрику тіків і зрілому вікні бокса."""
    dip = Obs(progress=progress(), box_pages=713, box_sec=900.0, underdeliver_streak=1)
    assert decide(dip, _target_cfg())[0] != "underdelivers"
    young = Obs(progress=progress(), box_pages=90, box_sec=300.0, underdeliver_streak=5)
    assert decide(young, _target_cfg())[0] != "underdelivers"


def test_without_a_target_the_promise_rule_is_silent() -> None:
    obs = Obs(progress=progress(), box_pages=713, box_sec=900.0, underdeliver_streak=9)
    assert decide(obs, CFG)[0] != "underdelivers"
...
def test_a_pipeline_queue_has_no_per_case_ramp_ups_in_the_forecast() -> None:
    """🔴 23.09.2026: конвеєр публікує `case_index=1` із 93, і прогноз додав
    92 розгони справ, яких у конвеєрі немає: «вийде $0.95» при фактичних
    $0.08 — бюджетний запобіжник обірвав захід на 60%."""
    base = progress(n_pages_expected=2008, pages_done=1224, pages_per_hour=3498,
                    wall_sec=1300.0, case_index=1, cases_total=93,
                    regulator={"rates": {"4": 3300}})
    piped = dict(base, pipeline=True)
    queue = Obs(progress=base, dph=0.143, spent_usd=0.053)
    pipe = Obs(progress=piped, dph=0.143, spent_usd=0.053)
    assert pipe.cases_left == 0
    assert pipe.projected_total_usd() < 0.2
    assert queue.projected_total_usd() >= pipe.projected_total_usd()
    assert decide(pipe, Cfg(budget_usd=1.0, max_hours=4.0))[0] != "destroy_budget"


def test_a_stale_window_rate_below_the_case_average_is_not_the_pace() -> None:
    """Захід spr-76 (24.09.2026): регулятор переніс із конвеєра дрібних справ
    замір `6 шардів → 1965`, а велика справа на тому ж флоті йшла 4018 у
    середньому (з розгоном). Прогноз від 1965 дав «$0.77 при бюджеті $0.80» і
    погасив здоровий захід; від сталого темпу виходить близько $0.45."""
    prog = progress(n_pages_expected=11500, pages_done=660, pages_per_hour=4018,
                    wall_sec=600.0, regulator={"active": 6, "rates": {"6": 1965, "10": 3003}})
    obs = Obs(progress=prog, dph=0.1364, spent_usd=0.076)
    assert obs.steady_pages_per_hour >= 4018
    assert obs.projected_total_usd() < 0.60
    assert decide(obs, Cfg(budget_usd=0.80, max_hours=11.0))[0] != "destroy_budget"


def test_a_box_whose_steady_pace_holds_the_target_is_not_underdelivering() -> None:
    """spr-80 (24.09.2026): сталий замір 3578 на 10 шардах, середнє від першої
    сторінки 2559 (розгін флоту й старт великої книги), поріг 0.85 × 3847."""
    from gpurunner.supervise.htr import underdelivering

    assert not underdelivering(1500, 2100.0, 3578.0, 3847.0)
    assert underdelivering(1500, 2100.0, 2600.0, 3847.0), "справді повільна — вирок"
    assert not underdelivering(150, 2100.0, 0.0, 3847.0), "замало сторінок для вироку"
