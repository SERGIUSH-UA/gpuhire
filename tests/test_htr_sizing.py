...
def _recorded_runs_of_this_machine(monkeypatch: pytest.MonkeyPatch) -> None:
    """Сторожі калібровки нижче читають СПРАВЖНІ заходи машини — лише читають.

    `conftest` ховає від тестів теку користувача, бо інші тести там писали;
    цей файл — виняток навмисний: на порожній теці сторожі пропускаються, і
    калібровка лишилась би без приймача. На CI даних немає — пропуск там і був.
    """
    monkeypatch.delenv("GPURUNNER_DATA_DIR", raising=False)
...
def test_cold_start_overhead_is_charged() -> None:
    ...
    # ⚠ Тут стояло `cold > 2 * warm`, і 23.09.2026 це зламалось від правильної
    # зміни моделі: накладні черги (`PER_CASE_SEC`, `PER_PAGE_OVERHEAD_SEC`)
    # входять В ОБА прогнози й розбавляють відношення. Перевіряємо те, про що
    # сам тест і написаний, — РІЗНИЦЮ холодного й теплого старту, і те, що на
    # дрібній справі вона важить більше за читання.
    reading = 100 / s.pages_per_hour
    assert cold - warm > reading, (
        "холодний старт на сотні сторінок мусить коштувати більше за читання"
    )


def test_big_case_on_reference_box_matches_measured_wall_clock() -> None:
    """1665 сторінок на V100 — разом зі стартом і транспортом (замір: 41 хв на
    1700 сторінок без накладних).

    ⚠ Верхня межа піднята з 1.10 до 1.17 год 23.09.2026, і не «бо не влазило»:
    модель навчилась рахувати транспорт, якого доти не бачила. Для 1665
    сторінок це 1665 × 0.138 с = 230 с плюс 27 с на справу, тобто рівно
    +0.071 год — саме на цю величину й піднято межу, без запасу. Самі накладні
    заміряні на трьох заходах 23.09: див. `PER_PAGE_OVERHEAD_SEC`.
    """
    s = plan_sizing(cores=64, vram_gb=32.0)
    hours = predict_hours(1665, s)
    assert 0.7 <= hours <= 1.1 + (1665 * hs.PER_PAGE_OVERHEAD_SEC
                                  + hs.PER_CASE_SEC) / 3600.0
...
def test_a_case_with_a_few_pages_per_shard_does_not_measure_the_pace() -> None:
    """Менше п'яти сторінок на шард — темп справи міряє старт шардів, а не
    читання: така справа в калібровку не йде, хоч би скільки сторінок мала."""
    from gpurunner.htr.calibrate import MIN_PAGES_PER_SHARD_FOR_RATE, Row, rated

    def row(pages: int, shards: int) -> Row:
        return Row(case=f"c{pages}-{shards}", out_dir="", gpu="g", n_gpus=1, cores=16,
                   vram=24, shards=shards, lines=40, pages=pages, actual=1000,
                   updated="2026-09-23T00:00", pages_this_run=pages, wall_sec=100)

    few = MIN_PAGES_PER_SHARD_FOR_RATE * 9 - 1
    kept = {r.case for r in rated([row(few, 9), row(few, 3), row(200, 9)])}
    assert kept == {f"c{few}-3", "c200-9"}
