...
class _ParamsOnly:
    ...
    #: Секрет складу народжується в `__init__` наглядача — тут його дає тест.
    _origin_token = "sekret-zakhodu"
...
def test_box_urls_are_in_the_params_before_any_rent() -> None:
    ...
    token = _ParamsOnly._origin_token
    assert params["pages_url"] == f"{bt.base_url()}/{token}/cases/sprava.tar"
    ...
    # 🔴 Секрет мусить бути в КОЖНІЙ адресі, і це не косметика: склад із
    # секретом відповідає 404 на будь-який шлях без нього, тож посилання без
    # секрету означає раннера, який не може забрати ні кадри, ні ассети, ні
    # чекпоінт — на вже оплаченій машині.
    for url in [params["pages_url"], params["assets_url"], *params["ckpt_urls"],
                *params["resume_urls"]]:
        assert f"/{token}/" in url, url
...
class _SyncOnly:
    ...
    _box_ckpt_dir = Supervisor._box_ckpt_dir
    _box_transport = Supervisor._box_transport
    _transfer_ceiling = Supervisor._transfer_ceiling
    #: Каналу ще не міряли — стеля переносу бере песимістичну підлогу.
    _uplink_mbs = 0.0
...
def _sync_rig(tmp_path: Path, monkeypatch, *, on_box: list[str]):
    ...
    def fake_exec(client: Any, cmd: str) -> str:
        ...
        if "find ." in cmd:
            return "\n".join(on_box)
        # Машина відповідає РОЗМІРОМ спакованого — з нього рахується стеля
        # часу на перенос. `yes` тут більше не годиться: стала стеля однаково
        # хибна і для 0.004, і для 20 МБ/с.
        return "4096"
...
class _DeliveryOnly:
    ...
    #: Замір лягає сюди й далі в реєстр; плече — те, яким справді везли.
    _origin_outside = ""
    _uplink_mbs = 0.0
    _uplink_via = ""
