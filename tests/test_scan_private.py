...
@pytest.mark.parametrize("rule_id, sample", [
    ...
    # Дефолтне ім'я тому з акаунта автора: у публічному пакеті воно читається
    # як «так і треба», і чужа людина заводить у себе том із назвою чужого роду.
    ("private-repo", '_VOLUME_NAME = "megen-spotter"'),
    ("private-space", 'out_root = tmp_path / "rodovid-shupyky" / "reports"'),
...
@pytest.mark.parametrize("clean", [
    ...
    "https://github.com/SERGIUSH-UA/gpurunner",
...
def test_allow_is_pointwise_not_blanket():
    ...
    # атрибуція автора в метаданих — так; те саме прізвище в коді — ні
    assert scan._allowed("pyproject.toml", "clan-surname")
    assert not scan._allowed("pyproject.toml", "contact-email")
    assert not scan._allowed("src/gpurunner/cli.py", "clan-surname")
    assert not scan._allowed("README.md", "private-repo")
...
def test_the_published_history_is_clean():
    ...
    Перевірено на живому випадку: стара назва CLI лишалась у чотирьох блобах
    двох комітів, тоді як обидва файли в дереві були вже виправлені.
    `test_this_repository_is_clean` цього не бачив за побудовою, і єдиним
    місцем, де провал спливав, був джоб CI — тобто вже після push, коли
    дешевого виправлення не лишається.

    🔄 21.09.2026 перевіряється ГІЛКА ПУБЛІКАЦІЇ, а не всі об'єкти репозиторію.
    Приватна історія розробки лишається приватною за рішенням автора: вона
    заражена від самого кореневого коміта (прізвище роду, шляхи машини, пошта),
    і чистити її означало б переписати 136 комітів заради того, чого ніхто не
    побачить. Публічна історія натомість починається чистим коренем — і саме
    там ціна пропуску незворотна.
    """
    ref = scan.public_ref()
    if not ref:
        pytest.skip("у цьому клоні немає гілки публікації "
                    f"(`{scan.PUBLIC_REF_DEFAULT}` або ${scan.PUBLIC_REF_ENV})")
    findings = []
    for rel, text, where in scan.iter_history(ref):
        findings += scan.scan_text(rel, text, where)
    assert not findings, "\n".join(
        f"{f.path}:{f.line_no} [{f.rule.id}] {f.excerpt}" for f in findings[:20])
...
def test_every_rule_has_a_sample_in_this_file():
    ...
    covered = {"canon-person", "canon-family", "canon-place", "canon-source",
               ...
               "private-repo", "private-space", "aws-presigned", "bearer", "private-host"}
