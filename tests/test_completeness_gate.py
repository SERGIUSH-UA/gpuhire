...
def _case_dir(root, model: str, n: int = 3):
    import json as _json

    d = root / "spr-2378"
    (d / "out").mkdir(parents=True)
    for i in range(n):
        (d / "out" / f"{i:04d}.txt").write_text("текст", encoding="utf-8")
    (d / "_htr_meta.json").write_text(_json.dumps({"model": model}), encoding="utf-8")
    (d / "htr_case_summary.json").write_text(_json.dumps(
        {"n_pages_expected": n, "complete": True}), encoding="utf-8")
    return d


def test_text_of_another_model_is_not_done(tmp_path) -> None:
    """🔴🔴 23.09.2026: 175 тек із латинським текстом Скриби пройшли як `done`
    для кириличного прогону Писарем — звірка рахувала будь-які `*.txt`."""
    from gpurunner.supervise.verify import verify_case

    d = _case_dir(tmp_path, "skryba_f792_v6.mlmodel")
    same = verify_case(d, expected_hint=3, model="skryba_f792_v6.mlmodel")
    other = verify_case(d, expected_hint=3, model="/kaggle/input/m/pysar_cyr_v17.pt")
    assert same.complete
    assert not other.complete
    assert "pysar_cyr_v17.pt" in other.detail and "skryba" in other.detail


def test_foreign_model_text_is_set_aside_not_mixed(tmp_path) -> None:
    """Розкладання пропускає наявні `*.txt`, а мету перезаписує — так v17 лягла
    поверх тексту v6. Старий текст не видаляється: він переїжджає поруч."""
    import json as _json

    from gpurunner.supervise.htr import _retire_foreign_model

    out = _case_dir(tmp_path / "home", "skryba_f792_v6.mlmodel")
    src = tmp_path / "staging" / "spr-2378"
    src.mkdir(parents=True)
    (src / "_htr_meta.json").write_text(_json.dumps({"model": "pysar_cyr_v17.pt"}),
                                        encoding="utf-8")
    _retire_foreign_model(out, src, lambda *_: None)
    assert not list((out / "out").glob("*.txt")), "старий текст мав звільнити місце"
    kept = list((out / "_prev_skryba_f792_v6.mlmodel").rglob("*.txt"))
    assert len(kept) == 3, "старий декод не видаляється"
    # та сама модель — нічого не рухається
    same = _case_dir(tmp_path / "same", "pysar_cyr_v17.pt")
    _retire_foreign_model(same, src, lambda *_: None)
    assert len(list((same / "out").glob("*.txt"))) == 3
