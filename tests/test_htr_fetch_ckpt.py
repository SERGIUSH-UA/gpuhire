...
def test_the_rescue_brings_home_the_segmentation_cache(tmp_path: Path) -> None:
    """🔴🔴 Тут стояло `continue` з коментарем «data/, logs/ — не декод».

    Рятунок через це викидав `data/derived/htr_seg/` — КЕШ СЕГМЕНТАЦІЇ. Наслідок
    видно не одразу й не в лозі: перечитування тієї ж справи другою моделлю
    мусить брати готову сегментацію (замір 18.4 → 9.1 с/стор, тобто ВДВІЧІ
    дешевше), шукає її на диску, не знаходить — і платить за сегментацію вдруге.
    У плані це тихо ставало «засіву немає».

    Штатний забір наглядача кеш приносить (`_remap` лишає `data/…` як є), тож
    рятунок мусить давати те саме: інакше «врятована» справа дорожча за забрану.
    """
    import gzip
    import json
    import tarfile
    ...
    tray = tmp_path / "box"
    (tray / "out").mkdir(parents=True)
    (tray / "out" / "0001.txt").write_text("текст", encoding="utf-8")
    (tray / "out-diak_v4").mkdir()
    (tray / "out-diak_v4" / "0001.txt").write_text("голос", encoding="utf-8")
    seg = tray / "data" / "derived" / "htr_seg" / "case"
    seg.mkdir(parents=True)
    with gzip.open(seg / "0001.o0.c400.seg.json.gz", "wt", encoding="utf-8") as fh:
        json.dump({"key": {"sato": "1,3", "max_endpoints": 400, "seg_height": 0}}, fh)
    (tray / "logs").mkdir()
    (tray / "logs" / "shard_00.log").write_text("лог шарда", encoding="utf-8")

    ball = tmp_path / "ckpt_0001.tgz"
    with tarfile.open(ball, "w:gz") as tf:
        for item in sorted(tray.rglob("*")):
            if item.is_file():
                tf.add(item, arcname=str(item.relative_to(tray)).replace("\\", "/"))

    out_root = tmp_path / "reports" / "spr-1"
    added = unpack(ball, "spr-1", out_root)

    assert added == {"out": 1, "out-diak_v4": 1}
    assert (out_root / "0001.txt").is_file(), "декод лягає пласко, як і раніше"
    assert (out_root.parent / "spr-1-diak_v4" / "0001.txt").is_file()
    cached = out_root / "data" / "derived" / "htr_seg" / "case" / "0001.o0.c400.seg.json.gz"
    assert cached.is_file(), "кеш сегментації знову викинуто — перечитування платить двічі"
    assert (out_root / "logs" / "shard_00.log").is_file(), (
        "логи шардів теж не декод, і саме в них лежить причина збоїв")


def test_a_dry_rescue_writes_nothing_at_all(tmp_path: Path) -> None:
    """Сухий прогін мусить лишатись сухим і для того, що поза гілками."""
    import tarfile

    tray = tmp_path / "box"
    (tray / "data" / "derived" / "htr_seg" / "case").mkdir(parents=True)
    (tray / "data" / "derived" / "htr_seg" / "case" / "0001.seg.json.gz").write_bytes(b"x")
    ball = tmp_path / "c.tgz"
    with tarfile.open(ball, "w:gz") as tf:
        tf.add(tray / "data" / "derived" / "htr_seg" / "case" / "0001.seg.json.gz",
               arcname="data/derived/htr_seg/case/0001.seg.json.gz")

    out_root = tmp_path / "reports" / "spr-2"
    unpack(ball, "spr-2", out_root, dry=True)
    assert not out_root.exists(), "сухий прогін нічого не пише"


def test_a_dry_rescue_reports_what_is_in_the_store(tmp_path: Path, monkeypatch) -> None:
    """Сухий прогін нічого не розпаковує, тож і диск для нього порожній; звіт
    мусить казати, скільки сторінок В АРХІВАХ (24.09.2026: «67 чекпоінтів і
    нулі» при повних книгах у сховищі)."""
    import shutil as _sh

    from gpurunner.htr import fetch_ckpt as fc

    first, second = tmp_path / "ckpt_0001.tgz", tmp_path / "ckpt_0002.tgz"
    _make_ckpt(first, branch="out", names=["0001.txt", "0002.txt"])
    _make_ckpt(second, branch="out", names=["0002.txt", "0003.txt"])
    monkeypatch.setattr(fc, "ckpt_keys", lambda *a, **k: [str(first), str(second)])
    monkeypatch.setattr(fc.r2, "get_url", lambda key, **k: key)
    monkeypatch.setattr(fc, "fetch_url", lambda url, dest, **k: _sh.copy(url, dest))
    out_root = tmp_path / "home" / "spr-1"
    got = fc.fetch_case({"case": "spr-1", "out_dir": str(out_root), "n_pages": 3},
                        dry=True, log=lambda _l: None)
    assert got["disk"] == {"out": 3} and got["ok"]
    assert not out_root.exists() or not any(out_root.rglob("*.txt"))
