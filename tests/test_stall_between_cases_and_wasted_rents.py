...
def test_download_floor_never_drops_to_ten_kilobytes() -> None:
    ...
    # Підлога перераховується НА КОЖНУ спробу — саме тут, бо тут живе цикл спроб.
    assert "_download_floor(attempt)" in body_of("_download_with_heartbeat")
    # 🔴 Фазу публікує той, хто чекає на curl. 22.09.2026 качання розділилось на
    # два шляхи — один потік і діапазонні з'єднання, — і серцебиття мусить бути
    # в ОБОХ: наглядач не розрізняє, яким саме шляхом іде качання, він бачить
    # лише те, чи оновлюється прогрес. Поведінку стереже сусідній тест;
    # цей — що жоден шлях не лишився без публікації взагалі.
    assert '"downloading"' in body_of("_download_stream")
    assert '"downloading"' in body_of("_watch")
