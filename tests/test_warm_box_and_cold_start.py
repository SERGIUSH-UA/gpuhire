...
def test_an_idle_box_picks_up_a_case_appended_after_the_queue_drained(
    ...
    # Годинник рухає лише `sleep`: інакше заключна тиша після довіску
    # чекається справжні 30 с.
    clock = {"t": 1000.0}
    monkeypatch.setattr(runner.time, "time", lambda: clock["t"])
    monkeypatch.setattr(runner.time, "sleep",
                        lambda s: clock.__setitem__("t", clock["t"] + max(s, 1)))
