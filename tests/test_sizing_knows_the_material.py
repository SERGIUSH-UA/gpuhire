...
def test_std160_box_gets_the_fleet_that_was_measured() -> None:
    ...
    # 11 ядер і 11 шардів за VRAM (24×0.9/1.95) сходяться в одній точці —
    # обидва обмежувачі чесні, головне, що жоден не дав п'ять
...
def test_light_frames_get_a_lower_vram_threshold() -> None:
    ...
    замість 12. На 16-Мпікс розворотах поріг лишається ~3.3, а невідомий
    матеріал і далі вважається важким."""
    assert 1.8 <= gb_per_shard_for(7.2) <= 2.1
    assert gb_per_shard_for(16.0) >= 3.3
