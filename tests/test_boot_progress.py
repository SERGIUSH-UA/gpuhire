...
def test_fetch_raises_and_drops_the_brake() -> None:
    ...
    # 🔴 Не вікно в символах: те саме `SRC[i:i + 4000]` уже зламалось від
    # ДОДАНОГО коментаря — `sftp.remove` виїхав за межу на один рядок, і тест
    # повідомив, що кран не знімається, хоч він знімається у `finally`.
    body = method_body(SRC, "fetch_outputs")
