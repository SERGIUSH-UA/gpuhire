...
def test_the_download_loop_takes_prefetched_frames_before_curl() -> None:
    ...
    take = body.index("prefetched.take(url, resume_to=local)")
...
def test_adoption_runs_after_resume_and_before_the_fleet() -> None:
    ...
    assert body.index("_resume_from_urls(work, resume_urls)") < adopt         < body.index("_start_shard(k, denom")


# ---- обрив не скидає привезеного -------------------------------------------


def test_a_broken_download_resumes_instead_of_starting_over(tmp_path, monkeypatch) -> None:
    """22.09.2026, spr-160: архів 2.25 ГБ на каналі 21.6 Мбіт/с тягнеться 14 хв,
    і кожен `rc=28` починав їх заново — три спроби по 14 хвилин оплаченого боксу.

    Тепер обрив коштує лише тієї хвилини, що впала: повтор продовжує з байта,
    який уже на диску.
    """
    monkeypatch.setattr(runner, "_set_phase", lambda phase, **kw: None)
    monkeypatch.setattr(runner, "DOWNLOAD_RETRY_SLEEP_SEC", 0)
    monkeypatch.setattr(runner, "_ranged_plan", lambda url, local: 1000)
    local = tmp_path / "big.tar"
    calls = {"stream": 0}

    def fake_stream(url, dst, name, floor, heartbeat, min_free, attempt, total=0):
        calls["stream"] += 1
        dst.write_bytes(b"x" * 400)          # привезли 400 з 1000 і впали
        return 28

    ranged = {}

    def fake_ranged(url, dst, name, total, parts, floor, hb, mfb, attempt, begin=0):
        ranged.update(total=total, parts=parts, begin=begin)
        with dst.open("ab") as fh:
            fh.write(b"y" * (total - begin))
        return 0

    monkeypatch.setattr(runner, "_download_stream", fake_stream)
    monkeypatch.setattr(runner, "_download_ranged", fake_ranged)

    assert runner._download_with_heartbeat("https://r2/big.tar", local, "big.tar") == 0
    # 🔴 Головне: продовжили з 400-го байта, а не качали тисячу наново.
    assert ranged["begin"] == 400
    assert local.stat().st_size == 1000
    assert calls["stream"] == 1, "після вдалого продовження другої спроби не треба"


def test_a_resume_that_fails_falls_back_to_a_clean_restart(tmp_path, monkeypatch) -> None:
    """Продовження — оптимізація, а не обов'язок: якщо воно не пішло, лишається
    перевірений шлях «з нуля», і недокачане прибирається."""
    monkeypatch.setattr(runner, "_set_phase", lambda phase, **kw: None)
    monkeypatch.setattr(runner, "DOWNLOAD_RETRY_SLEEP_SEC", 0)
    monkeypatch.setattr(runner, "_ranged_plan", lambda url, local: 1000)
    local = tmp_path / "big.tar"
    ...
    def fake_stream(url, dst, name, floor, heartbeat, min_free, attempt, total=0):
        ...
        if attempt == 1:
            dst.write_bytes(b"x" * 400)
            return 28
        dst.write_bytes(b"z" * 1000)
        return 0

    monkeypatch.setattr(runner, "_download_stream", fake_stream)
    monkeypatch.setattr(runner, "_download_ranged",
                        lambda *a, **kw: 28)          # продовження теж падає
    assert runner._download_with_heartbeat("https://r2/big.tar", local, "big.tar") == 0
    assert seen["attempts"] == [1, 2]
    assert local.read_bytes() == b"z" * 1000, "після невдалого продовження — чистий старт"
